...
- Repository Exact commit/comparison：只接受精确 object ID，最多显示 200 条 changed path/status/mode metadata；比较行可分别预览或在同一只读区域成对显示既有 base/head 脱敏 regular-file preview，不显示 author/email/body/blob/remote/root，不 checkout 或修改 ref；
...
- Verify association：操作者显式把一条 later evidence 关联到一个 earlier plan item；每项只显示 pass/fail/unknown 计数与 frozen snapshot-keyset 分页，并可下载 digest-bound Markdown/JSON metadata snapshot 或显式记录不可变 metadata receipt；回执历史不含正文/操作者身份且固定不是 snapshot/result acceptance，矛盾证据保留且不推导整份计划通过；
