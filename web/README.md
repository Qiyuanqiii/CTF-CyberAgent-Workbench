...
The current UI includes controlled Run creation, Run/Session browsing, redacted model availability, explicit Plan/Delivery, a metadata-only approval queue, steering counts and ordered metadata, Session enqueue/pending cancellation, Start/Pause/Resume, an at-most-eight-step Run Queue action, Agent/delegation/Fan-out/Finding views, resumable events, Work/Notes/Artifact/ToolRound metadata, budgets/leases, execution profiles, external-Skill provenance, separately gated Diff apply and foreground wake consumption, and Desktop-confirmed inert Skill installation. Bearers and intent-bound retry keys stay in page memory and never enter a URL or browser storage. TypeScript submits no backend, scope, gate, private lease, path, file body, tool argument, or authority field; model/message bodies and private identities are omitted from browser DTOs.
