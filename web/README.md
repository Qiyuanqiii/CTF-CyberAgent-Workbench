...
schema v45 的 Run 概览会只读显示操作者引导队列的 pending/prepared/committed/cancelled 计数和有界消息元数据。schema v46 的本地取消与 drain 不向 Web 开放。非 schema D1-S1 只在独立 capability 下增加现有 Run-bound Session 的 enqueue/replay：浏览器响应不接收正文、摘要、操作者或内部投递身份，也没有编辑、重排、取消、唤醒或 drain 按钮。
...
这是 CyberAgent Workbench 的本地 read-mostly 运维界面。React/Vite 只消费 Go 生成的 OpenAPI 3.1 DTO 和 `api.v1`，不会重新实现 Policy、审批、工作区权限、Shell、Docker、模型路由或 SQLite 逻辑。当前可见 mutation 只有 schema v64 的非授权 Run 执行档位选择、schema v72 的受控 Run 创建和非 schema D1-S1 的受控 Session 消息排队。
...
浏览器不持久化 token，也不把 token 放入 URL、body 或静态资源请求。可选 control token 只驻留页面内存，当前界面用它提交幂等的执行档位 ID、受控 Run 创建或受控 Session message；Go 决定 Workspace/Run/Session 绑定、默认预算、backend、scope、审批、风险、门禁和全部权限位。不确定失败后的 operation key 只在同一内存表单与未修改正文下复用，不进入浏览器存储。生产模式由 Go 在同一回环 origin 托管已构建资源；Vite 回环代理只用于前端开发，跨域代理目标会被拒绝。Artifact 正文、checkpoint pending input、操作者引导响应正文/摘要/身份、外部 Skill 正文/路径/摘要/私有身份、raw Fan-out report、审批/生命周期私有叙述、lease/fencing token、进程启动和其他执行类写操作仍不向 Web 开放。

同一 bundle 也被 Desktop D0-A/D0-B/D1-R1/D1-S1 编译进 Wails v2.13.0 Windows 壳。React 通过三个方法的严格 native bridge 自动取得内存连接材料并调用进程内 Go API，不监听端口、不使用 Local Storage。Windows AssetServer 不支持 response streaming，因此 Desktop 调用 `run-event-poll.v1`，消费与 SSE 相同的 Run-bound 高水位 cursor 和真实 frame；最多在模块内存保留 16 个 Run、每个 500 帧，浏览器仍使用 SSE。WebView2 在打开 bundle/数据库前只读预检，失败时不下载或安装；精确 renderer origin、外部链接/表单/popup 阻断和 Wails binding-origin allowlist 共同形成导航防线。桌面专属 Skill 按钮只调用无参数原生 `.zip` picker，React 从不接收路径或 bytes，也没有安装按钮。New Run 和 Session composer 分别只在 Go bootstrap 返回对应独立 capability 时出现。
