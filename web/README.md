...
同一 bundle 也被 Desktop D0-A/D0-B 编译进 Wails v2.13.0 Windows 壳。React 通过三个方法的严格 native bridge 自动取得内存连接材料并调用进程内 Go API，不监听端口、不使用 Local Storage。Windows AssetServer 不支持 response streaming，因此 Desktop 调用 `run-event-poll.v1`，消费与 SSE 相同的 Run-bound 高水位 cursor 和真实 frame；最多在模块内存保留 16 个 Run、每个 500 帧，浏览器仍使用 SSE。WebView2 在打开 bundle/数据库前只读预检，失败时不下载或安装；精确 renderer origin、外部链接/表单/popup 阻断和 Wails binding-origin allowlist 共同形成导航防线。桌面专属 Skill 按钮只调用无参数原生 `.zip` picker，React 从不接收路径或 bytes，也没有安装按钮。
