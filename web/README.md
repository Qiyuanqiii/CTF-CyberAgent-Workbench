...
- Repository Exact commit/comparison：只接受精确 object ID，最多显示 200 条 changed path/status/mode metadata；比较行可分别预览或在同一只读区域成对显示既有 base/head 脱敏 regular-file preview，并在有界变化集合内上一项/下一项导航；不显示 author/email/body/blob/remote/root，不 checkout 或修改 ref；
