...
- Repository Exact commit/comparison：只接受精确 object ID，最多显示 200 条 changed path/status/mode metadata；比较行可分别复用既有 base/head 脱敏 regular-file preview，不显示 author/email/body/blob/remote/root，不 checkout 或修改 ref；
...
- Verify association：操作者显式把一条 later evidence 关联到一个 earlier plan item；每项只显示 pass/fail/unknown 计数与 frozen snapshot-keyset 分页，并可下载 digest-bound Markdown/JSON metadata snapshot；矛盾证据保留且不推导整份计划通过；
