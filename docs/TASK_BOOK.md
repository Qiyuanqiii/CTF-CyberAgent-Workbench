...
- 产品可用度：完整 Code + Cyber 产品约 70-74%。
- 通用 Coding Agent 工作流可用度：约 66%。
...
P7 已推进到 schema v71 与非 schema D1-B1，P9/Desktop 已推进到 schema v76：schema v41-v47 完成工作面/阶段、Plan/Delivery、来源隔离、切片门禁、操作者队列和内置 Specialist Skill；schema v69-v71 完成惰性用户 Skill Registry、精确 Run 选择/最小化上下文和三端安全来源投影；D1-A/D0-A/D0-B 完成路径隔离预览、Wails Windows 壳、同库恢复、高水位续传和 WebView2/origin 失败关闭；schema v72-v74 完成幂等 Run/Session/lifecycle/有界交接、模型/Plan/审批、Diff 审阅与 wake intent；schema v75-v76 再完成显式前台 wake 消费和独立 Diff apply；D1-B1 允许 HTTP/Desktop 显式确认后登记惰性 Skill 包。各项由独立 capability 控制，不建立 Desktop-native worker，也不授予通用 Shell、LocalRunner、Docker、后台执行、安装 hook 或子 Agent 能力。
...
- [ ] Desktop D1（当前产品可用度约 70-74%）：下一批完成 operation receipt/recovery 统一呈现、Go-owned 有界只读 Workspace explorer 与便携构建诊断；累计六片后执行完整健壮性门。
