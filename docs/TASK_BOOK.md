...
- 产品可用度：完整 Code + Cyber 产品约 80-84%。
- 通用 Coding Agent 工作流可用度：约 77%。
...
P7 已推进到 schema v71 与非 schema D1-B1，P9/Desktop 已推进到 schema v77 与 D1-K1：schema v41-v47 完成工作面/阶段、Plan/Delivery、来源隔离、切片门禁、操作者队列和内置 Specialist Skill；schema v69-v71 完成惰性用户 Skill Registry、精确 Run 选择/最小化上下文和三端安全来源投影；D1-A/D0-A/D0-B 完成路径隔离预览、Wails Windows 壳、同库恢复、高水位续传和 WebView2/origin 失败关闭；schema v72-v76 完成幂等 Run/Session/lifecycle/有界交接、模型/Plan/审批、Diff 审阅/apply、wake intent/前台消费；D1-B1 允许 HTTP/Desktop 显式确认后登记惰性 Skill 包；D1-U1/E1/W1 完成统一恢复回执、有界只读 Workspace Explorer 与可复现便携构建诊断；schema v77 D1-E2/C1/U2 完成有界搜索、操作者显式非授权证据附加和可刷新终态回执历史；非 schema D1-O1/C2/K1 再完成有界操作者行动中心、metadata-only 已附加证据清单和只导航/刷新的命令面板。各项由独立 capability 或 read-only 契约控制，不建立 Desktop-native worker，也不授予通用 Shell、LocalRunner、Docker、后台执行、安装 hook、renderer host path 或子 Agent 能力。
...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消、非授权 Run 执行档位选择、schema-v72 受控 Run 创建、Session 排队/pending-only 取消、schema-v73 Run 生命周期/有界 Supervisor 交接、脱敏模型可用性、显式在线诊断与持久化路由、Plan 三选一/Deliver、受限审批、schema-v74 wake 意图、schema-v75 显式前台消费、schema-v76 Diff 独立审阅/apply、D1-B1 惰性 Skill 安装、D1-U1/U2 统一回执与历史、D1-E1/E2 有界 Workspace explorer/search、schema-v77 D1-C1 非授权证据附加、D1-W1 便携构建诊断、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE 与同 cursor `run-event-poll.v1`、`headless.v1` NDJSON、Run-first TUI、React/Vite 控制台和 Windows Wails 壳已落地；Desktop Provider 密钥设置、后台 wake worker、统一行动中心/快捷命令与 Windows 10 实机矩阵尚待完成，计划见 `docs/DESKTOP_PLAN.md`
...
- [ ] Desktop D1（当前产品可用度约 80-84%）：下一批先完成 D1-I1 Go-issued proposal/diff editor、D1-M3 Go/OS-owned Provider secret settings boundary，以及 D1-J1 默认关闭的 bounded wake worker；每项先过独立威胁模型，renderer 不得直接写文件/读密钥，worker 不得启动 Shell/Local/Docker。
