...
状态：进行中；schema v24 Specialist Attempt Runtime 已完成，child 模型执行与 root inbox context 尚未开放
...
- [ ] 让 root 通过有界、恰好一次的 context projection 消费 dependency/result/notification inbox；模型不得直接控制消息游标。
- [ ] 将一个内部 Specialist no-tool 模型 turn 接入现有 Provider/Supervisor 预算与取消边界；保持 CLI/HTTP/model spawn 关闭。
