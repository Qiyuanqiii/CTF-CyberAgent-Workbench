...
状态：进行中；只读 `skill.v1` Registry、schema v39-v47 内置 Skill/模式/Plan/来源/交付/队列链、非 schema 的 `skill_package.v1` 严格校验、schema v69 内容寻址惰性用户 Registry、schema v70 外部 Skill 的不可变 Run 选择与最小上下文，以及 schema v71 HTTP/TUI/Web metadata-only 来源投影均已完成；下一推荐切片为 Desktop D1-A 的 Go-owned 本地验证预览，计划见 `docs/SKILL_PACKAGE_PLAN.md`
