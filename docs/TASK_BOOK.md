...
- 产品可用度：完整 Code + Cyber 产品约 55-59%。
- 通用 Coding Agent 工作流可用度：约 49%。
...
P7 已推进到 schema v71，P9/Desktop 已推进到 schema v72 与 D1-R1：schema v41-v47 完成工作面/阶段、Plan/Delivery、来源隔离、切片门禁、操作者队列和内置 Specialist Skill；schema v69 增加内容寻址、不可变、默认不授权的用户 Skill 安装 Registry；schema v70 再以第二次显式确认固定精确 active 外部版本，并把脱敏正文仅作为用户角色的非可信工作流指导最小化交付给 root 和最多一个操作者指定的 Specialist；schema v71 将有界且不含正文、路径、摘要或私有身份的来源/交付元数据只读投影到 HTTP、TUI 和 Web；D1-A 固定一次性路径隔离预览句柄，D0-A 通过 Wails v2.13.0 原生 `.zip` 对话框把该预览安全接入 Windows Desktop，D0-B 再补齐同库恢复、高水位续传和 WebView2/origin 失败关闭；schema v72/D1-R1 增加显式 capability 下的幂等受控 Run 创建与自动 Session 绑定。模式、提案、选择、文档、审计声明、排队、取消、Skill 安装、预览和 Run 创建都不授予工具、网络、Shell、写文件或子 Agent 能力。
...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消、非授权 Run 执行档位选择与 schema-v72 受控 Run 创建、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE 与同 cursor `run-event-poll.v1`、`headless.v1` NDJSON 与稳定终态退出码、持久事件驱动的 Run-first TUI、React/Vite 控制台、显式 Go 同源生产托管，以及 Windows Desktop D0-A/D0-B/D1-R1 Wails 壳、生命周期/并发/恢复/WebView2/origin 门禁已落地；D1 Session/Plan/审批/Diff/Skill mutation 与 Windows 10 实机矩阵尚待完成，计划见 `docs/DESKTOP_PLAN.md`
...
- [ ] Desktop D1-S1：增加现有 Run-bound Session 的 Go-owned message/steering mutation；复用 RunSupervisor、持久化队列、预算、Policy、脱敏、operation key 和事件语义，不建立 Desktop-only 执行路径。
