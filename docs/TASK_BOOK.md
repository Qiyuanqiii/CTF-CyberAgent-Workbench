...
V2 的 98.5% 在 P0/P1 基础上完成了可恢复 Supervisor、预算、严格生命周期、Run-bound Session、Provider typed outcome/retry/SSE、有界 `model.delta`、active-call 查询/取消/订阅、Bubble Tea 实时取消，以及一次可跨重启恢复的协议修复。P3 已加入 schema v9 Work Board、schema v10 Notes、事务关系/事件、`todo`/`note` CLI、可见性规则、8192-token Context Section 选择和 `model.started` 来源审计。P5 已统一工作区读取、Shell、FileEdit 与 workspace-scoped `script_process.v1` 提案入口，并新增 schema v11 持久化幂等审批账本、schema v12 可撤销 Session Grant 与原子工具预算、schema v13 独立脚本进程提案、schema v14 脱敏且来源绑定的 Run 输出 Artifact，以及 schema v15 create-only WorkItem/Note 结构化工具与幂等操作账本。

费用预算、跨进程控制、Coordinator 和真实 Sandbox 尚未实现；Run 工具预算已在 P5 落地。
...
状态：主体完成；Provider ToolCall 调度循环待接入
...
状态：主体完成；结构化创建工具已接入，模型自动调度待 P2/P5 联调
...
- [ ] 将 Provider ToolCall 适配到 RunSupervisor 的受限工具循环；当前只允许 CLI/Go 调用结构化工具，模型仍不能自动修改 Work Board/Notes。
...
状态：进行中；Gateway、持久化逐次审批、Session Grant、工具预算、first-class ScriptProcess、Run 输出 Artifact 与 create-only 结构化记忆工具已完成
...
- [ ] 把 Provider ToolCall 的 arguments/tool-call ID 映射到 Gateway payload/operation key，并在 RunSupervisor 中实现有界、可恢复、仅允许白名单结构化工具的循环。
