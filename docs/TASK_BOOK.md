...
V2 的 58% 在 P0/P1 基础上完成了 schema v6 Supervisor checkpoint、`RunSupervisor`/`RunHandle`/`LifecycleResult`、单次无工具 root Agent turn、崩溃恢复、累计 token/模型执行时间预算、有界执行循环和原子终态。结构化 root 生命周期协议、费用/工具预算、Coordinator、结构化任务板和真实 Sandbox 尚未实现。
...
- [ ] 实现模型驱动的 root `run.finish`、`agent.wait` 和有界恢复协议；当前终态只接受操作者显式命令。
