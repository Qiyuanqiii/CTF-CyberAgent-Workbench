...
状态：进行中；schema v32 核心委派保持最多两个 child，schema v33 已完成独立只读 Fan-out 规划门，下一步是只读执行门与显式核心调度入口
...
- [ ] 为 v33 增加只读执行状态机：执行前逐文件复核 snapshot，最多六个无工具 JSON worker，共享 Run 总预算、Provider 并发门和取消扇出，结果结构化汇总且不具备写入/网络/再委派权限。
