...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消、非授权 Run 执行档位选择、schema-v72 受控 Run 创建、Session 排队/pending-only 取消、schema-v73 Run 生命周期/有界 Supervisor 交接、脱敏模型可用性、Plan 三选一/Deliver、受限审批、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE 与同 cursor `run-event-poll.v1`、`headless.v1` NDJSON、Run-first TUI、React/Vite 控制台和 Windows Wails 壳已落地；Desktop Provider 密钥设置/在线诊断、Diff/Skill mutation、后台 scheduler 与 Windows 10 实机矩阵尚待完成，计划见 `docs/DESKTOP_PLAN.md`
...
- [ ] Desktop D1（当前产品可用度约 64-68%）：继续通过 Go mutation API 增加 Provider 诊断/路由设置、Diff 和 Skill 管理与后台 scheduler；关闭窗口不取消 Run，CLI/Desktop 并发与重启恢复通过审计。
