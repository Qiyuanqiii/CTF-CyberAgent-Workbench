...
状态：进行中；schema v30 受限 Specialist delegation proposal 已完成，下一步实现 operator/internal review 与可恢复 application
...
- [ ] 为 schema v30 proposal 增加独立 operator/internal review fact 和可恢复 application：批准后重新复核 Policy、容量、预算和 parent Skills，原子或可幂等恢复地接入现有 admission，并向每个 child 写入严格父指令；拒绝、过期和部分应用不得留下幽灵 child。
