...
- 产品可用度：完整 Code + Cyber 产品约 48-52%。
- 通用 Coding Agent 工作流可用度：约 43%。
...
P7 已推进到 schema v70：schema v41-v47 完成工作面/阶段、Plan/Delivery、来源隔离、切片门禁、操作者队列和内置 Specialist Skill；schema v69 增加内容寻址、不可变、默认不授权的用户 Skill 安装 Registry；schema v70 再以第二次显式确认固定精确 active 外部版本，并把脱敏正文仅作为用户角色的非可信工作流指导最小化交付给 root 和最多一个操作者指定的 Specialist。模式、提案、选择、文档、审计声明、排队、取消、Skill 安装和选择都不授予工具、网络、Shell、写文件或子 Agent 能力。
...
状态：进行中；只读 `skill.v1` Registry、schema v39-v47 内置 Skill/模式/Plan/来源/交付/队列链、非 schema 的 `skill_package.v1` 严格校验、schema v69 内容寻址惰性用户 Registry，以及 schema v70 外部 Skill 的不可变 Run 选择与最小上下文均已完成；下一推荐切片为 v71 HTTP/TUI/Web metadata-only 来源投影，计划见 `docs/SKILL_PACKAGE_PLAN.md`
