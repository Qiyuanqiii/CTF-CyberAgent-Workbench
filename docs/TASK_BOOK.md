...
V2 的 52% 在 P0/P1 基础上增加了 schema v5 Supervisor checkpoint、`RunSupervisor`/`RunHandle`/`LifecycleResult`、单次无工具 root Agent turn、崩溃恢复、turn budget 与取消前置边界。自动执行循环、累计预算、Coordinator、结构化任务板和真实 Sandbox 尚未实现。
...
- [ ] 增加累计 token、金额、时间和工具调用预算；turn budget 已落地。
