...
- 产品可用度：完整 Code + Cyber 产品约 57-61%。
- 通用 Coding Agent 工作流可用度：约 52%。
...
P7 已推进到 schema v71，P9/Desktop 已推进到 schema v72 与 D1-S1：schema v41-v47 完成工作面/阶段、Plan/Delivery、来源隔离、切片门禁、操作者队列和内置 Specialist Skill；schema v69 增加内容寻址、不可变、默认不授权的用户 Skill 安装 Registry；schema v70 再以第二次显式确认固定精确 active 外部版本，并把脱敏正文仅作为用户角色的非可信工作流指导最小化交付给 root 和最多一个操作者指定的 Specialist；schema v71 将有界且不含正文、路径、摘要或私有身份的来源/交付元数据只读投影到 HTTP、TUI 和 Web；D1-A 固定一次性路径隔离预览句柄，D0-A 通过 Wails v2.13.0 原生 `.zip` 对话框把该预览安全接入 Windows Desktop，D0-B 再补齐同库恢复、高水位续传和 WebView2/origin 失败关闭；schema v72/D1-R1 增加显式 capability 下的幂等受控 Run 创建与自动 Session 绑定；非 schema D1-S1 以独立 capability 把现有 Run-bound Session 输入持久化到 v45-v46 队列。提交本身不启动/恢复/drain Run，不调用模型或工具，也不授予工具、网络、Shell、写文件或子 Agent 能力。
...
schema v65 已增加不可变 `sandbox_docker_production_evidence.v1`：Go 固定 16 项机器 probe 和摘要协议，CLI 只接受同一操作者的 v63 阻塞审查、稳定操作键和显式确认。schema v66 再增加 collector 调用前持久化的 attempt、摘要化 operation、过期 generation lease、当前代 quiescent reconciliation、类型化 failure 和原子 result。schema v67 只在 Linux 显式 opt-in 后执行五次固定 GET，增加不可变 harness intent、daemon-aware empty-scope reconciliation 和零生产验证 result；schema v68 再增加一次不可变操作员接纳/拒绝决定。所有 start/process/export/Artifact authority 继续为 false。schema v69-v71 已完成外部 Skill 惰性 Registry、Run 固定选择/最小化加载和三端安全只读来源投影；Desktop D1-A 路径隔离、D0-A/D0-B 可见壳与恢复门禁、schema-v72 D1-R1 受控 Run 创建和非 schema D1-S1 Session message submission 均已完成。下一批三个切片是 pending-only steering 取消、operator Run lifecycle 控制和 Go-owned bounded execution handoff，并在第六片后执行完整健壮性门。
...
状态：进行中；只读 `skill.v1` Registry、schema v39-v47 内置 Skill/模式/Plan/来源/交付/队列链、非 schema 的 `skill_package.v1` 严格校验、schema v69 内容寻址惰性用户 Registry、schema v70 外部 Skill 的不可变 Run 选择与最小上下文、schema v71 HTTP/TUI/Web metadata-only 来源投影、Desktop D1-A 路径隔离 Go 预览桥，以及 D0-A/D0-B 原生只读预览与恢复加固均已完成；Desktop/HTTP 安装 mutation 仍后置独立审计，计划见 `docs/DESKTOP_PLAN.md`
...
- [ ] Desktop D1-B 在 D0-A 原生预览之后另行增加 Go-owned 确认安装 mutation；TypeScript 不提交路径、不解压、不校验、不写 Registry。任何 HTTP/control 接口使用独立 token、Origin/Host、大小、幂等和取消门禁，并单独审计。
...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消、非授权 Run 执行档位选择、schema-v72 受控 Run 创建和非 schema D1-S1 Session 消息排队、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE 与同 cursor `run-event-poll.v1`、`headless.v1` NDJSON 与稳定终态退出码、持久事件驱动的 Run-first TUI、React/Vite 控制台、显式 Go 同源生产托管，以及 Windows Desktop D0-A/D0-B/D1-R1/D1-S1 Wails 壳、生命周期/并发/恢复/WebView2/origin 门禁已落地；D1 queue cancel/Run lifecycle/execution handoff、Plan/审批/Diff/Skill mutation 与 Windows 10 实机矩阵尚待完成，计划见 `docs/DESKTOP_PLAN.md`
...
- [x] Desktop D1-A Go 边界：原生 selector 与 renderer bridge 分离，后者没有接受路径/文件字节的方法；D0-A 已通过原生 `.zip` 对话框和一次性句柄接入 React/桌面可执行文件，仍不提供 HTTP upload 或安装 mutation。
...
- [ ] Desktop D1-S2：增加独立 pending-only steering cancellation HTTP/UI；复用 v46，prepared/committed/cancelled 状态不可改写。
- [ ] Desktop D1-L1：增加 digest-idempotent operator Run start/pause/resume 控制；精确状态、quiescence、lease 与 capability 门独立于消息提交。
- [ ] Desktop D1-X1：增加 Go-owned bounded execution handoff，把已排队输入交给既有 RunSupervisor/预算/Policy/lease/model/event 路径，不建立 Desktop-native 执行器。
- [ ] 六切片健壮性门：在 D1-S2/D1-L1/D1-X1 完成后运行全仓 ordinary/race、vet、staticcheck、govulncheck、依赖/契约/隐私检查、Windows/Vite 构建和重启/并发功能测试。
