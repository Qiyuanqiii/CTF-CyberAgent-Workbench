...
P7 已推进到 schema v43：schema v41 为每个 Run 固定 `code|cyber` 工作面与 `plan|deliver` 阶段；schema v42 增加严格三方向 `plan_delivery.v1` 提案、操作者幂等选择，以及向既有 WorkItems/置顶 decision Note 的原子投影；schema v43 再以 `context_provenance.v1` 将操作者指令与文件、工具、Diff、记忆等非可信证据分离。方向选择后 Run 仍处于 Plan，必须另行显式进入 Deliver。模式、提案、选择和文档内容都不授予工具、网络、Shell、写文件或子 Agent 能力。
...
状态：进行中；只读 `skill.v1` Registry、schema v39 Run 选择、schema v40 root 上下文交付、schema v41 Go-owned Run 模式、schema v42 Plan 提案/操作者选择与 schema v43 Session/context 来源隔离已完成
...
- [ ] 在 schema v44 增加 Delivery 切片门禁：绑定被选 WorkItem、验收条件、聚焦测试、diff/安全审计、交接 Note，并在较大模块结束后执行整体验证与健壮性门禁。
