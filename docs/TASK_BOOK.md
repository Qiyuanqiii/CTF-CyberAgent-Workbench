...
V2 的 72% 在 P0/P1 基础上完成了 schema v7 Supervisor checkpoint、单次无工具 root Agent turn、崩溃恢复、累计 token/模型执行时间预算、有界执行循环、严格 `continue/finish/wait` root 生命周期协议、Run-bound Session chat 统一路径，以及 Provider typed outcome、有限重试和模型终态事件。真实 token stream、协议自动修复、费用/工具预算、Coordinator、结构化任务板和真实 Sandbox 尚未实现。
...
- [ ] 增加无效协议的有界自动修复、结构化依赖等待和未来 child `agent.finish`；当前无效输出记录失败并等待重试。
- [ ] 增加金额和工具调用预算；turn、token 和模型执行时间预算已落地。
- [ ] 增加跨进程主动取消控制；context 取消、Provider 限流稳定返回和进程重启恢复已落地。
- [ ] 增加真实 `model.delta` stream；错误分类、重试、超时和模型终态事件已统一。
