...
状态：进行中；schema v29 durable schedule summary 与跨进程 child-call cancellation 已完成，下一步实现受限 Specialist delegation proposal
...
- [ ] 定义受限 `specialist_delegation.v1` proposal：root 最多提出两个有界目标、Skill 子集和预算建议；Go 复核并持久化 proposal，第一阶段仍要求内部/operator policy 决定，模型不能直接 admission 或 spawn。
