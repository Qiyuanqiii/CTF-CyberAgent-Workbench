...
- 产品可用度：完整 Code + Cyber 产品约 61-65%。
- 通用 Coding Agent 工作流可用度：约 56%。
...
P7 已推进到 schema v71，P9/Desktop 已推进到 schema v73 与 D1-X1：schema v41-v47 完成工作面/阶段、Plan/Delivery、来源隔离、切片门禁、操作者队列和内置 Specialist Skill；schema v69-v71 完成惰性用户 Skill Registry、精确 Run 选择/最小化上下文和三端安全来源投影；D1-A/D0-A/D0-B 完成路径隔离预览、Wails Windows 壳、同库恢复、高水位续传和 WebView2/origin 失败关闭；schema v72/D1-R1 增加幂等受控 Run 创建；D1-S1/S2 增加现有 Run-bound Session 的排队和 pending-only 取消；schema v73/D1-L1/D1-X1 增加幂等 start/pause/resume 与最多八条冻结输入的显式 RunSupervisor 交接。最后三项彼此独立，不建立 Desktop-native worker，也不授予 Shell、LocalRunner、Docker、网络、文件写入、审批或子 Agent 能力。
...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消、非授权 Run 执行档位选择、schema-v72 受控 Run 创建、Session 排队/pending-only 取消、schema-v73 Run 生命周期/有界 Supervisor 交接、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE 与同 cursor `run-event-poll.v1`、`headless.v1` NDJSON、Run-first TUI、React/Vite 控制台和 Windows Wails 壳已落地；Desktop Provider 配置、Plan/审批/Diff/Skill mutation 与 Windows 10 实机矩阵尚待完成，计划见 `docs/DESKTOP_PLAN.md`
...
- [ ] Desktop D1-M1：复用 CLI 的 Go-owned Provider Registry/模型路由初始化并投影脱敏可用性；API key 不进入 TypeScript、SQLite 或公开事件。
- [ ] Desktop D1-P1：通过独立 digest-idempotent Go controls 开放 Plan 三选一和显式 Plan-to-Deliver；模型不能代选，选择不能自动执行。
- [ ] Desktop D1-A1：通过独立 capability 投影现有审批队列并开放 approve-once/deny；永久 Policy 拒绝、Session Grant 范围和 process-disabled 结果不可绕过。
