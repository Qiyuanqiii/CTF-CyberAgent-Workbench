...
P8 已开始：schema v35 将完成的 Fan-out execution 确定性投影为通用 `draft` Finding、不可变 `model_assertion` Evidence 和 `finding_report.v1` Report，支持不调用模型的 Markdown/JSON 输出。
...
状态：进行中；schema v32 核心委派保持最多两个 child，schema v33/v34 已完成独立只读 Fan-out 规划与执行门，下一步是通用 Finding/Report 汇总与显式核心调度入口
...
状态：进行中；schema v35 已完成第一版不可变 draft 投影、Evidence 来源绑定与 Markdown/JSON，验证状态迁移和 SARIF 待后续切片

- [x] 定义通用 Finding 类型、`draft/validating/validated/accepted/fixed/rejected` 状态枚举；v35 仅允许创建不可变 `draft`，状态迁移尚未开放。
...
- [ ] 增加 Finding 验证状态迁移、Artifact-backed Evidence、SARIF 和 CI annotations。
