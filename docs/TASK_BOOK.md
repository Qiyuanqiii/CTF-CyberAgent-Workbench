...
- [ ] P10 下一批：A1 Go-owned analyzer protocol/limits/errors 且无 process starter；A2 Rust `clap`/`serde` 确定性 stdin JSON -> stdout JSON fixture tool；A3 Go/Rust shared golden vectors 与 CI，产品调用和 Artifact commit 继续关闭。本机 Rust toolchain 待按既有依赖授权安装并记录版本。
