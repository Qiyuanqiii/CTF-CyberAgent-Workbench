...
- 产品可用度：完整 Code + Cyber 产品约 52-56%。
- 通用 Coding Agent 工作流可用度：约 46%。
...
P7 已推进到 schema v71 加非 schema Desktop D1-A/D0-A：schema v41-v47 完成工作面/阶段、Plan/Delivery、来源隔离、切片门禁、操作者队列和内置 Specialist Skill；schema v69 增加内容寻址、不可变、默认不授权的用户 Skill 安装 Registry；schema v70 再以第二次显式确认固定精确 active 外部版本，并把脱敏正文仅作为用户角色的非可信工作流指导最小化交付给 root 和最多一个操作者指定的 Specialist；schema v71 将有界且不含正文、路径、摘要或私有身份的来源/交付元数据只读投影到 HTTP、TUI 和 Web；D1-A 固定一次性路径隔离预览句柄，D0-A 则通过 Wails v2.13.0 原生 `.zip` 对话框把该预览安全接入 Windows read-first Desktop。模式、提案、选择、文档、审计声明、排队、取消、Skill 安装、预览和选择都不授予工具、网络、Shell、写文件或子 Agent 能力。
...
状态：进行中；只读 `skill.v1` Registry、schema v39-v47 内置 Skill/模式/Plan/来源/交付/队列链、非 schema 的 `skill_package.v1` 严格校验、schema v69 内容寻址惰性用户 Registry、schema v70 外部 Skill 的不可变 Run 选择与最小上下文、schema v71 HTTP/TUI/Web metadata-only 来源投影，以及非 schema Desktop D1-A 路径隔离 Go 预览桥均已完成；下一推荐切片为 Desktop D0 Wails 只读壳和原生文件对话框接线，计划见 `docs/DESKTOP_PLAN.md`
...
- [x] Desktop D1-A 增加路径隔离 Go 预览桥：原生 picker 只把路径交给 Go closure；严格校验后仅发放最多 16 个、五分钟过期、单次消费的 256-bit 句柄，renderer DTO 不含路径/正文/description/content path/content digest，且全部安装/执行/联网/模型/工具授权为 false；D0-A 已接通可见入口，边界见 ADR 0033/0034。
...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消与非授权 Run 执行档位选择、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE、`headless.v1` NDJSON 与稳定终态退出码、持久事件驱动的 Run-first TUI、React/Vite 控制台、显式 Go 同源生产托管，以及 Windows Desktop D0-A read-first Wails 壳已落地；D0-B 生命周期/并发/恢复和 D1 业务 mutation 尚待完成，计划见 `docs/DESKTOP_PLAN.md`
...
- [x] 提供经过单独审计的主动取消入口；独立 control token 不能读取，read token 不能取消，客户端不能提供 fencing token。schema v64 复用该独立 capability 增加仅记录操作者意图的档位选择；它不启动进程、不授予 backend 或 capability。除取消与档位选择外 HTTP API 保持只读；WebSocket 只在未来确有双向或模型正文需求时再引入。
...
- [x] Desktop D1-A 前置 Go 边界：原生 selector 与 renderer bridge 分离，后者没有接受路径/文件字节的方法；当前不接入生产启动、HTTP/OpenAPI/React，也不创建桌面可执行文件。
...
- [x] Desktop D0-A 本地发布门：全仓普通/race 205.1 秒/293.9 秒，ordinary/desktop-tag vet、staticcheck、零漏洞 govulncheck，31 项前端测试、build/npm audit、61 份 Markdown/82 条链接及隐私扫描通过；Desktop 聚焦 50 轮普通/10 轮 race，最终未签名 GUI SHA-256 为 `6b355cfa72b41d225e62ed58ac24cb9493bbf2a71f4d45120e6f0dbf5308ad0c`。远端 Windows/Go/Web 三路 CI 待推送后记录。
- [ ] Desktop D0-B：完成 CLI/Desktop 同库并发、崩溃重开、单实例恢复、event polling cursor 续传、WebView2 缺失诊断和 production navigation/binding-origin 自动化；继续只读，不做安装器、注册表、自启动、更新或后台服务。
