...
- 产品可用度：完整 Code + Cyber 产品约 94-96%。
- 通用 Coding Agent 工作流可用度：约 94%。
...
- [ ] Desktop D1（当前产品可用度约 94-96%）：下一批 D1-G5 有界脱敏 exact-commit file preview、D1-V4 Handoff/export verification coverage metadata 与 R2 test-only 非产品 process-tree conformance adapters；完成后执行当前六切片周期的 full race/staticcheck/govulncheck/依赖/隐私/构建门，仍不开放 unrestricted `os/exec`、Local/Docker product start 或 xterm 输入。
