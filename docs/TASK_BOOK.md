...
P7 的模式地基已推进到 schema v41：每个 Run 由 Go 固定 `code|cyber` 工作面与 `plan|deliver` 执行阶段。工作面、Profile、Scope 与策略绑定在 Run 内不可变；阶段只能由操作者在 `created` 或无活动 execution lease 的 `paused` 状态下显式切换。Plan 完成在 Supervisor、应用、Store 与 SQLite 边界失败关闭。模式与权限正交，不会授予工具、网络、Shell、写文件或子 Agent 能力。
...
状态：进行中；只读 `skill.v1` Registry、schema v39 Run 选择、schema v40 root 上下文交付与 schema v41 Go-owned Run 模式已完成
...
- [ ] 在 v41 之上增加 Plan/Delivery 内置 Skill：三方向选择、接受方案、模块/切片推进、切片审计、交接 Note 与大模块健壮性门禁，复用 WorkItems/Notes。
