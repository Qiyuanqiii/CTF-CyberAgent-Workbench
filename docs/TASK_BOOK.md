...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE、`headless.v1` NDJSON 与稳定终态退出码、第一版 Run-aware TUI、React/Vite Run/Agent/delegation/Fan-out/Finding 只读控制台及显式 Go 同源生产托管已落地
