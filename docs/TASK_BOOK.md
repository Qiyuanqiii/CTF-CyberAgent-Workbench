...
- 产品可用度：完整 Code + Cyber 产品约 49-53%。
- 通用 Coding Agent 工作流可用度：约 44%。
...
P7 已推进到 schema v71 加非 schema Desktop D1-A：schema v41-v47 完成工作面/阶段、Plan/Delivery、来源隔离、切片门禁、操作者队列和内置 Specialist Skill；schema v69 增加内容寻址、不可变、默认不授权的用户 Skill 安装 Registry；schema v70 再以第二次显式确认固定精确 active 外部版本，并把脱敏正文仅作为用户角色的非可信工作流指导最小化交付给 root 和最多一个操作者指定的 Specialist；schema v71 将有界且不含正文、路径、摘要或私有身份的来源/交付元数据只读投影到 HTTP、TUI 和 Web；D1-A 再固定未来 Desktop 只能通过 Go 原生 selector 发放的一次性不透明句柄读取风险预览。模式、提案、选择、文档、审计声明、排队、取消、Skill 安装和选择都不授予工具、网络、Shell、写文件或子 Agent 能力。
...
- [x] Desktop D1-A 增加路径隔离 Go 预览桥：未来原生 picker 只把路径交给 Go closure；严格校验后仅发放最多 16 个、五分钟过期、单次消费的 256-bit 句柄，renderer DTO 不含路径/正文/description/content path/content digest，且全部安装/执行/联网/模型/工具授权为 false；边界见 ADR 0033。
- [ ] Desktop D1-B 增加可见原生文件选择与确认安装；TypeScript 不解压、不校验、不写 Registry。任何回环上传 API 使用独立 control token、Origin/Host、大小和幂等门禁，并单独审计。
