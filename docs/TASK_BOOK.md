...
状态：进行中；schema v31 独立 Specialist delegation review 已完成，下一步实现可恢复 application
...
- [ ] 为 approved proposal 增加可恢复 application：重新复核 Policy、容量、预算和 parent Skills，原子或可幂等恢复地接入现有 admission，并向每个 child 写入严格父指令；过期和部分应用不得留下幽灵 child。
