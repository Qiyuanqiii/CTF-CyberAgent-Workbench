...
- [ ] P10-B1/B2/B3：实现无 executable path/starter 的惰性 analyzer descriptor Registry、memory-only/no-extraction `archive.inventory.v1` 协议，以及 Rust ZIP inventory 与共享对抗向量；产品调用继续关闭，并在批末完成当前六切片完整健壮性门。
...
- [ ] 实现惰性 Analyzer Descriptor Registry；描述符只能声明精确协议/限制/全 false capability，不能保存 executable path 或启动进程。
- [ ] 首个分析器选择 archive inventory：只检查内存 ZIP central directory，不解压、不写文件，并固定 traversal/duplicate/oversize/ratio 风险向量。
