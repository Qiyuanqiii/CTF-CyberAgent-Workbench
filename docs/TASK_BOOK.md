...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE、第一版 Run-aware TUI 与 React/Vite 只读控制台已落地
...
- [ ] 由 Go 在显式配置下同源托管生产 Web bundle，补齐 route-aware CSP、SPA fallback 上限和静态资源集成测试。
- [ ] 在 Go DTO/契约先行的前提下增加 Agent graph、delegation/Fan-out 与 Finding/Report 只读 Web 视图。
