...
- [ ] Desktop D1（当前产品可用度约 94-96%）：下一批建议 D1-G5 有界脱敏 exact-commit file preview、D1-V4 Handoff/export verification coverage metadata 与 R2 test-only 非产品 process-tree conformance adapters；仍不开放 unrestricted `os/exec`、Local/Docker product start 或 xterm 输入。
