...
- 旧版 v0.1 通用 Agent 骨架：约 97%。
- V2 Run-centric Runtime：约 34%。
- 完整产品愿景：约 42%。

V2 的 34% 包含可复用的 Session、LLM Router、Workspace、Policy、ToolRun、FileEdit、Context Compaction 和 TUI，以及新完成的版本化 migration、Mission/Run 状态机、append-only Run Events 和 CLI 生命周期。Coordinator、结构化任务板、恢复执行和真实 Sandbox 尚未实现。
...
- [ ] 把 Session、ToolRun、FileEdit 事件投影到统一 Run 时间线。
- [ ] 为旧 `agent.Task` 提供兼容映射。
...
- [ ] 定义 `RunSupervisor`、`RunHandle`、`LifecycleResult`。
- [ ] 把现有 Session chat 接入 Run，而不是直接调用 Router。
- [ ] 实现 root `run.finish`、`agent.wait` 和有界恢复协议。
- [ ] 增加 turn、token、金额、时间和工具调用预算。
- [ ] 支持用户取消、Provider 限流暂停和进程重启恢复。
- [ ] 统一模型 stream event、错误分类、重试和超时。
...
- [ ] 定义 `WorkItem` 状态、优先级、依赖、Owner 和验收条件。
- [ ] 定义 `Note` 分类、标签、Evidence 引用和 Agent 可见性。
- [ ] 新增 `work_items`、`notes` 表和事件。
- [ ] 增加 `todo create/list/update/complete/block` 工具与 CLI。
- [ ] 增加 `note create/list/get/update` 工具与 CLI。
- [ ] Context Builder 只加载活跃任务和按需 Notes。
