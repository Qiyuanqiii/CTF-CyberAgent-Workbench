...
状态：进行中；Go 内部最多两个 child 的有界调度、取消扇出与总预算复核已完成，下一步实现 child lifecycle repair
...
- [ ] 为 child 无效 lifecycle 增加恰好一次有界 repair；repair 与 transport retry 分开计数，原始坏输出不进入 Session、事件或 repair prompt。
