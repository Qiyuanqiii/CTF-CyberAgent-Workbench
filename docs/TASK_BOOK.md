...
- 产品可用度：完整 Code + Cyber 产品约 46-50%。
- 通用 Coding Agent 工作流可用度：约 41%。
...
P7 已推进到 schema v69：schema v41 为每个 Run 固定 `code|cyber` 工作面与 `plan|deliver` 阶段；schema v42 增加严格三方向 `plan_delivery.v1` 提案与操作者幂等选择；schema v43 将操作者指令与非可信证据分离；schema v44 固化逐切片验证、审计和交接门禁；schema v45-v46 提供安全 turn 边界的 exactly-once 操作者队列、pending-only 取消和显式 drain；schema v47 从父 Run 固定选择中为每个 Specialist Attempt 派生最多一项、Code/Cyber 分离、metadata-only 可恢复的内置 Skill 指导；schema v69 再增加内容寻址、不可变、默认不授权的用户 Skill 安装 Registry。模式、提案、选择、文档、审计声明、排队、取消和 Skill 存储本身都不授予工具、网络、Shell、写文件或子 Agent 能力。
...
状态：进行中；只读 `skill.v1` Registry、schema v39 Run 选择、schema v40 root 上下文交付、schema v41 Go-owned Run 模式、schema v42 Plan 提案/操作者选择、schema v43 来源隔离、schema v44 Delivery 检查点门禁、schema v45-v46 操作者引导队列与控制、schema v47 Specialist 最小 Skill 上下文、非 schema 的 `skill_package.v1` 严格校验，以及 schema v69 内容寻址惰性用户 Registry 已完成；下一推荐切片为 v70 外部 Skill 的不可变 Run 选择与最小上下文加载，计划见 `docs/SKILL_PACKAGE_PLAN.md`
...
- [x] v69 最终门禁通过全仓普通/race（265.4 秒/260.7 秒）、vet/staticcheck/module/govulncheck、21 项前端测试、OpenAPI/build/npm audit、定向三轮 race，以及真实双 Service 独立候选身份 20 轮普通/10 轮 race 收敛。审计修复旧 schema 夹具移除顺序、对象收据绑定、发布前取消点、生成 ID/时间误判冲突、Manifest description 凭据脱敏和低风险静态问题，未发现未解决高/中风险；边界见 ADR 0031。
- [ ] schema v70 将外部包纳入 Run 的不可变精确版本选择；只允许未 tombstone 且对象通过回读验证的版本，声明工具不授予能力。
- [ ] 以独立 hash/Profile/预算/脱敏/来源账本向 root/Specialist 最小化交付外部 Skill，并保持 Code/Cyber 隔离、first-model-call 原子绑定和跨重启恢复。
...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消与非授权 Run 执行档位选择、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE、`headless.v1` NDJSON 与稳定终态退出码、持久事件驱动的 Run-first TUI、React/Vite Run/Agent/delegation/Fan-out/Finding 控制台及显式 Go 同源生产托管已落地；Windows Desktop 分阶段路线已确定但尚未实现，计划见 `docs/DESKTOP_PLAN.md`
