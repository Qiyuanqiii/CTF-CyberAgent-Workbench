...
- 产品可用度：完整 Code + Cyber 产品约 67-71%。
- 通用 Coding Agent 工作流可用度：约 63%。
...
P7 已推进到 schema v71，P9/Desktop 已推进到 schema v74 与 D1-Q1：schema v41-v47 完成工作面/阶段、Plan/Delivery、来源隔离、切片门禁、操作者队列和内置 Specialist Skill；schema v69-v71 完成惰性用户 Skill Registry、精确 Run 选择/最小化上下文和三端安全来源投影；D1-A/D0-A/D0-B 完成路径隔离预览、Wails Windows 壳、同库恢复、高水位续传和 WebView2/origin 失败关闭；schema v72-v73 完成幂等 Run 创建、Session 排队/取消、Run 生命周期与有界 Supervisor 交接；D1-M1/P1/A1 完成脱敏模型、Plan/Deliver 和受限审批；D1-M2/D1-D1 继续增加显式无正文 Provider 诊断、持久化路由与只审阅不 apply 的 Diff；schema v74/D1-Q1 固定有界 wake/retry 意图和单 owner fencing。各项由独立 capability 控制，不建立 Desktop-native worker，也不授予 Shell、LocalRunner、Docker、后台执行或子 Agent 能力。
...
schema v65 已增加不可变 `sandbox_docker_production_evidence.v1`：Go 固定 16 项机器 probe 和摘要协议，CLI 只接受同一操作者的 v63 阻塞审查、稳定操作键和显式确认。schema v66 再增加 collector 调用前持久化的 attempt、摘要化 operation、过期 generation lease、当前代 quiescent reconciliation、类型化 failure 和原子 result。schema v67 只在 Linux 显式 opt-in 后执行五次固定 GET，增加不可变 harness intent、daemon-aware empty-scope reconciliation 和零生产验证 result；schema v68 再增加一次不可变操作员接纳/拒绝决定。所有 start/process/export/Artifact authority 继续为 false。schema v69-v74 与 Desktop D1-A 至 D1-Q1 已完成外部 Skill 安全链、桌面恢复、日常 Run 控制、模型/Plan/审批、Diff 审阅和持久化 wake 意图。本批累计六切片完整健壮性门已通过：最终 ordinary/race 为 278.6/296.1 秒，静态/漏洞/依赖/隐私、80 项前端测试和 Windows/Vite 构建均为绿色；下一批转向显式前台 wake 消费、独立 Diff apply 与 Skill 安装控制。
...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消、非授权 Run 执行档位选择、schema-v72 受控 Run 创建、Session 排队/pending-only 取消、schema-v73 Run 生命周期/有界 Supervisor 交接、脱敏模型可用性、显式在线诊断与持久化路由、Plan 三选一/Deliver、受限审批、只审阅不 apply 的 Diff、schema-v74 wake 意图、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE 与同 cursor `run-event-poll.v1`、`headless.v1` NDJSON、Run-first TUI、React/Vite 控制台和 Windows Wails 壳已落地；Desktop Provider 密钥设置、独立 Diff apply、Skill mutation、后台 wake worker 与 Windows 10 实机矩阵尚待完成，计划见 `docs/DESKTOP_PLAN.md`
...
- [ ] Desktop D1（当前产品可用度约 67-71%）：下一批通过独立 Go mutation API 增加显式前台 wake 消费、已批准 Diff apply 和 Skill 安装；每项分别复核状态、幂等、权限、预算与重启恢复。
