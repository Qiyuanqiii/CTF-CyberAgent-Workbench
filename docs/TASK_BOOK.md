...
P7 已推进到 schema v71 与非 schema D1-B1，P9/Desktop 产品面已推进到 schema v82 与 D1-G5/V4，通用运行时安全面已推进到 H1-H3/R2/C1-C3：除既有 Run/Session/Plan/审批、Workspace/证据/回执/行动中心外，FileEdit 支持安全恢复和多文件独立审阅，Windows Credential Manager 可触发 generation-safe Provider Registry 原子 reload，普通浏览器与 Desktop 共享 metadata-only capability/worker health，并新增 Go-owned 只读仓库状态/脱敏 Diff/历史/精确提交预览、不可变操作者验证与覆盖率、可恢复 Code Handoff/导出及累计上下文记忆。H1-H3 增加 Tool 硬超时/特殊文件拒绝、有界同步等待图与可恢复 Run 无进展熔断；R2 仅在测试中验证真实 OS 进程树回收。各项由独立 capability 或 read-only 契约控制；renderer 不提交 host path、不能回读密钥，worker 不持有 Tool Runner，也不授予通用 Shell、LocalRunner、Docker、Git hook、安装 hook 或子 Agent 能力。
...
- [ ] Desktop D1（当前产品可用度约 95-97%）：下一批 D1-G6 bounded exact-file history、D1-V5 read-only verification coverage drill-down 与 R3 bounded output/exit-evidence contract；本批只做普通三切片门，仍不开放 unrestricted `os/exec`、Local/Docker product start 或 xterm 输入。
