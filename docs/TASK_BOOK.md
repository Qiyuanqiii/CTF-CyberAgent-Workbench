...
schema v65 已增加不可变 `sandbox_docker_production_evidence.v1`：Go 固定 16 项机器 probe 和摘要协议，CLI 只接受同一操作者的 v63 阻塞审查、稳定操作键和显式确认。当前 Windows/Linux collector 只记录 unsupported、opt-in required 或 harness pending，零 daemon/网络/进程调用；Application 拒绝任何 complete/real-daemon 结果，所有 start/process/export/Artifact authority 继续为 false。外部 Skill 安装顺延到 v66 或以后，预计 v67+。
...
状态：进行中；schema v48-v63 已完成从 Manifest 到阻塞态 Docker start-gate 设计审查，schema v64 只增加非授权 backend 档位选择，schema v65 只增加零副作用、非授权的生产证据捕获账本，真实 daemon harness 与进程执行仍未开放
...
- [ ] schema v66 在任何真实 daemon 调用前建立 durable write-ahead evidence-capture attempt、过期 generation lease、固定 Linux endpoint、typed failure 与重启协调；未持有当前 generation 的 worker 不能调用或提交。
- [ ] 在 v66 写前门禁后实现并审计 Linux 16-probe harness：只使用已存在的精确 digest 镜像、不 pull；receipt 仍不授权 start，必须再经过独立 evidence acceptance review。
- [ ] 本地代码默认只读挂载，输出目录独立可写。
...
状态：进行中；只读 `skill.v1` Registry、schema v39 Run 选择、schema v40 root 上下文交付、schema v41 Go-owned Run 模式、schema v42 Plan 提案/操作者选择、schema v43 来源隔离、schema v44 Delivery 检查点门禁、schema v45-v46 操作者引导队列与控制、schema v47 Specialist 最小 Skill 上下文，以及非 schema 的 `skill_package.v1` 只读校验已完成；schema v64-v65 已用于执行档位与 Docker 证据账本，自定义技能包导入/安装/上传顺延到 v66 或以后，预计 v67+，计划见 `docs/SKILL_PACKAGE_PLAN.md`
...
- [ ] schema v66 或后续（预计 v67+）实现 content-addressed 用户 Skill Registry、不可变安装/卸载账本、原子导入与恢复，以及 `skill import/installed/remove`；同名同版本异内容冲突，已被 Run 固定的版本不可删除。
