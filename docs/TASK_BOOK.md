...
状态：部分完成；loopback-only `api.v1` 读取面、独立授权的 root/Specialist 活动调用取消、Go 生成的 OpenAPI 3.1 契约、有界 Run-event SSE、`headless.v1` NDJSON 与稳定终态退出码、持久事件驱动的 Run-aware TUI、React/Vite Run/Agent/delegation/Fan-out/Finding 只读控制台及显式 Go 同源生产托管已落地
...
- [ ] 增加独立 Run 列表与 diff 视图；完整 Finding/Evidence 详情继续由现有 CLI/Web 按需读取。
...
验收标准：CLI、TUI、CI、Web 对同一 Run 显示一致状态；关闭 UI 不会停止后台 Run。当前已有 CLI/TUI/HTTP/Headless golden 固定 Run/Mission/Session/status/event tail/Agent count 一致性。
