...
- 产品可用度：完整 Code + Cyber 产品约 84-87%。
- 通用 Coding Agent 工作流可用度：约 82%。
...
P7 已推进到 schema v71 与非 schema D1-B1，P9/Desktop 已推进到 schema v77 与 D1-I1/M3/J1：除既有 Run/Session/Plan/审批、Workspace/证据/回执/行动中心外，D1-I1 增加 Go-issued source handle 的本地 Monaco FileEdit 提案，D1-M3 增加 Windows Credential Manager Provider 设置，D1-J1 增加默认关闭、单并发、单步的 wake worker。各项由独立 capability 或 read-only 契约控制；renderer 不提交 host path、不能回读密钥，worker 不持有 Tool Runner，也不授予通用 Shell、LocalRunner、Docker、安装 hook 或子 Agent 能力。
...
schema v65 已增加不可变 `sandbox_docker_production_evidence.v1`：Go 固定 16 项机器 probe 和摘要协议，CLI 只接受同一操作者的 v63 阻塞审查、稳定操作键和显式确认。schema v66 再增加 collector 调用前持久化的 attempt、摘要化 operation、过期 generation lease、当前代 quiescent reconciliation、类型化 failure 和原子 result。schema v67 只在 Linux 显式 opt-in 后执行五次固定 GET，schema v68 再增加一次不可变操作员接纳/拒绝决定。所有 start/process/export/Artifact authority 继续为 false。schema v69-v77 与 Desktop D1-A 至 D1-I1/M3/J1 已完成外部 Skill 安全链、桌面恢复、日常 Run 控制、模型/Plan/审批、FileEdit 提案/审阅/apply、持久化 wake/有界 worker、惰性 Skill、Workspace evidence、恢复回执、行动中心和键盘导航。SQLite 保持 v77。
...
- [ ] Desktop D1（当前产品可用度约 84-87%）：下一批完成 D1-I2 安全编辑恢复、D1-M4 Provider Registry generation reload、D1-J2 metadata-only browser capability + worker health/drain；补齐普通 `api serve` 的保守 discovery 缺口后累计六片，执行完整 race/vet/staticcheck/govulncheck/依赖/隐私门。
