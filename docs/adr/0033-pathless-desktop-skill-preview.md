...
are implemented, but no desktop executable, Wails binding, native file dialog,
HTTP upload, installation mutation, or installer exists yet. SQLite remains at
schema v71.

## 中文摘要

Desktop D1-A 复用现有严格 `skill_package.v1` 解析器，但不把本地文件路径
交给 TypeScript 或浏览器。未来原生文件选择器把路径只交给一个 Go 闭包；Go
...
The project does not yet have a desktop shell. This slice therefore establishes
the native boundary before a Wails integration can accidentally expose paths.
It intentionally does not claim that an end user can open a desktop preview UI
today.
...
The new package is not referenced by the CLI startup, HTTP server, OpenAPI,
React application, or any production command. It has no Store, model, Tool
Gateway, Sandbox, Docker, process, or network call. It reuses the repository's
typed error package, whose compile-time dependency graph includes HTTP and SQL
error mappings, but the bridge itself creates neither an HTTP handler nor a
database connection.

A future Wails adapter must bind only a method that opens the native dialog and
then invokes the Go selector. It must not bind a method accepting a renderer-
provided path, expose the selector closure, or add a JSON/HTTP path parameter.
...
Desktop D0 should next validate and pin the Wails shell, embed the existing
read-first React console, and add the native dialog adapter without adding an
installer. The preview bridge can then become visible through that native-only
flow. Any package installation or HTTP upload remains a separate mutation
slice requiring an independent control token, strict Host/Origin and content
type, streaming bounds, idempotency, CSRF resistance, cancellation, and audit.
