...
状态：Desktop D0-A、D0-B 与 D1-R1 至 schema-v77 D1-I2/M4/J2 自动化核心已完成。Wails v2.13.0 Windows 壳、嵌入式 React bundle、进程内 Go API、同库恢复、高水位事件续传、WebView2 失败关闭、内存令牌、原生 `.zip` 对话框、路径隔离 Skill 预览/惰性安装、受控 Run/Session/Plan/审批、安全恢复的 Monaco FileEdit 提案/Diff、generation-safe Windows Credential Manager Provider reload，以及默认关闭、单并发/单步、可观测 drain 的 wake worker 已经落地；Windows 10 实机矩阵、xterm、安装包、签名正式发行、注册表、自启动、更新和高权限执行仍未实现。
...
- [ ] 下一批候选 D1-G1 Go-owned 只读 repository-state projection、D1-I3 bounded multi-file change-set review 与 D1-F1 Code-mode 端到端旅程。
...
ADR 0034 至 ADR 0046 分别记录可见壳、生命周期/事件续传、受控 Run 创建、Session message submission、Run 控制/有界交接、模型/Plan/审批、Provider/Diff/wake intent、前台 wake/独立 apply/惰性 Skill 安装、统一回执/Workspace evidence/便携构建诊断、Workspace search/non-authorizing attachment/receipt history、operator actions/evidence inventory/command palette、Go-issued editor/system credentials/bounded worker，以及安全编辑恢复/Provider generation/worker health。Wails 使用 MIT 许可证；D2 生成任何可分发 ZIP/MSIX 前必须把 Wails 及其他运行时依赖的许可证/notice、SBOM 和哈希一起打包。
