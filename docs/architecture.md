...
The current root implementation maps `wait` to a durable paused Run and resumes at the next turn, and it permits one bounded automatic repair for an invalid root response. The child `agent.finish` persistence contract, lease-fenced scheduler, usage accounting, and bounded root inbox path now exist as internal Go boundaries. The next slice may connect one no-tool child Provider turn, but no model or public client can invoke spawn/finish and structured child dependency waiting remains future Coordinator work.
