...
1. 为 schema v31 approved proposal 增加可恢复 application：重新复核 Policy、capacity、budget、Skills，幂等接入现有 admission，并向 child 写入严格父指令；过期与中断恢复不能留下幽灵 child，模型仍不能直接 admission 或 spawn。
