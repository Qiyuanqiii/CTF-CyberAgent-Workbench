...
下一批固定为 P10-B1 无 executable path/starter 的 Go-owned `analyzer_descriptor.v1` 惰性注册表、
P10-B2 只检查内存 ZIP central directory 且不解压的 `archive.inventory.v1` 有界协议、P10-B3 Rust
ZIP inventory 与 benign/traversal/duplicate/oversize/ratio 共享对抗向量。产品调用、结果持久化和
Artifact commit 继续不接线。该批完成当前六切片周期，届时执行完整 race/staticcheck/govulncheck/
依赖/隐私健壮性门。真实 Local/Docker start、Windows 10 人工矩阵、签名分发、xterm、网络授权和
CTF 继续独立设门。
...
长对话恢复时依次阅读：`README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_STATUS.md`、本文件、`docs/TASK_BOOK.md`、`docs/http-api.md`、`docs/errors.md`，再按序阅读 `docs/adr/0001-*.md` 到 `docs/adr/0062-go-owned-analyzer-protocol-rust-fixture-shared-vectors.md`。
