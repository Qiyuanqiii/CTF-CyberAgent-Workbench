...
1. 定义严格 `specialist_delegation.v1` proposal：root 最多提出两个目标、Skill 子集和预算建议，Go 持久化并复核；第一阶段仍要求 internal/operator policy 批准，不让模型直接 admission 或 spawn。
