...
- V2 Run-centric Runtime：约 83%。
- 项目骨架和模块边界：约 99%。

V2 的 P0/P1 已完成，P2 第八纵向切片已落地：Go application 层现在拥有并发安全的 active-call registry、幂等审计取消和有界元数据订阅，CLI 入口也会把终端信号传入 Provider context。下一步让 Bubble Tea 消费实时状态并提供不退出程序的显式取消操作。
...
- Bubble Tea 对 active-call 元数据订阅的实时展示，以及不退出 TUI 的显式取消操作。
...
- `staticcheck ./...` 仅报告 3 个既存 TUI 低严重度问题：`S1008`、`S1011` 和未使用 helper `U1000`；本轮 active-call 代码无命中。
...
最新验证在原有 streaming 基线上新增覆盖：激活前不可见、同 Run 重复调用冲突、活动查询/列表、订阅 snapshot/progress/terminal、慢消费者断开、重复取消幂等、取消原因脱敏、`model.cancel_requested -> model.delta -> model.failed(cancelled)` 审计链，以及 `ExecuteContext` 终端取消后保留可恢复 checkpoint。
...
1. 给 Bubble Tea 注入只读 active-call 查询/订阅控制器，展示 provider、attempt、chunk/byte 和取消状态。
2. 增加不退出 TUI 的显式取消键，调用 application audit-first API；关闭 UI 仍不隐式取消后台 Run。
3. 为订阅断开和慢消费者状态增加清晰 TUI 呈现，并保持异步 Bubble Tea command 不阻塞 Update。
4. 在 metadata-only 路径稳定后设计经过 Go 生命周期投影与跨 chunk 脱敏的用户可见文本 streaming，不能直接转发原始协议 JSON。
