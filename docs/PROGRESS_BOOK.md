...
1. 增加独立 TUI Run 列表与有界、只读的文件 diff/detail 视图；写入和审批仍只走现有 Go service。
2. 把 CLI/TUI/Web/Headless golden 扩展到 paused/completed/failed/cancelled 与分页边界，避免界面各自解释 lifecycle。
