...
- CGO SQLite 驱动 `github.com/mattn/go-sqlite3`，当前 schema 版本为 v62。
...
桌面端与自定义 Skill 导入现已正式进入计划但尚未实现。Desktop 采用 D0-D3 分段：当前只准备 Wails + 现有 React + Go HTTP/SSE/OpenAPI 的开发/便携只读壳；产品可用度约 65-70% 后开放 Go-owned mutation，约 75-80% 后才制作便携 ZIP 与签名 MSIX，企业 MSI、自定义协议、自启动、服务和自动更新继续独立审计。现有 `skill.v1` 与内部 `LoadFS` 只能验证内嵌/测试 FS，不是用户导入入口；后续 `skill_package.v1` 将先做 Markdown-only 有界包校验，再做 content-addressed 原子安装和 CLI，最后接 Desktop 的 Go-owned 上传预览。两份计划分别记录在 `docs/DESKTOP_PLAN.md` 与 `docs/SKILL_PACKAGE_PLAN.md`，且不改变 schema v64 的当前优先级。
...
长对话恢复时依次阅读：`README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_STATUS.md`、本文件、`docs/TASK_BOOK.md`、`docs/http-api.md`、`docs/errors.md`，再按序阅读 `docs/adr/0001-*.md` 到 `docs/adr/0022-retained-runtime-input-resource-lifecycle.md`。
