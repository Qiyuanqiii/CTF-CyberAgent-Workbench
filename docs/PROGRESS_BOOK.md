...
1. 从稳定 Go DTO 生成并测试 OpenAPI 契约，为 Run/Session/Event/WorkItem/Note/Artifact/ToolRound/lease read DTO 建立唯一协议来源。
2. 在 execution lease 基础上设计只读 WebSocket/Event stream 与经过单独授权的跨进程取消接口，再建立 React/Vite UI；TypeScript 不重复实现校验或安全策略。
