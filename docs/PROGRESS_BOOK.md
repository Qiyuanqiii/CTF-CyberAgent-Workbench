...
V2 已完成版本化 migration、Mission/Run 状态机、append-only 事件主干和 CLI 生命周期。下一步继续复用现有 Session、ToolRun、FileEdit、Context、Workspace 与 Policy，不做一次性重写。
...
- Session、ToolRun、FileEdit、Policy activity 到 `run_events` 的兼容投影。
...
最新验证已覆盖 Run create/start/pause/resume/cancel/show/events 生命周期、migration 升级与回滚、文件编辑审批、工具 dry-run、工作区边界、上下文压缩、模型路由、TUI 快照和敏感信息脱敏。

## 七、下一开发切片

1. 增加稳定的 typed error codes，保持现有人类可读 CLI 错误不变。
2. 为 Run 关联或创建 Session，并记录关联关系。
3. 将 Session Message、ToolRun、FileEdit 和 Policy Decision 投影到 `run_events`。
4. 增加 legacy `agent.Task -> Mission/Run` 幂等适配器。
5. 保持 `run start` 仅推进生命周期，多 Agent 并发继续关闭，直到 RunSupervisor 到来。
