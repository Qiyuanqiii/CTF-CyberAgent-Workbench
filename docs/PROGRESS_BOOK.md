...
CLI 新增 `run delivery checkpoint/list/show` 唯一写入/检查入口；HTTP/OpenAPI、生成 TypeScript DTO、React Plan 面板和 Bubble Tea Plan 面板只显示是否强制、必需/就绪数量及有界 checkpoint 元数据。公共投影不含验证/审计正文、内部指纹、摘要、operation key 或 requester。模型没有 checkpoint 工具；Policy 对 Shell/Sandbox/process/script 中明显自调用 operator 命令的请求永久拒绝。审计未发现未解决的高/中风险问题，并在收口修复三个低风险边界：固定编号标题保证最大合法模块标题不会撑爆交接 Note；关系 UPDATE trigger 同时检查 OLD/NEW note 归属，不能把其他 Note 的关系搬入；Run event 删除不必要的验收/来源/交接摘要。

v44 发布门禁覆盖领域协议、应用端到端、两 Store 并发、SQLite 直接篡改/迁移、两条完成路径、真实 CLI 生命周期、Policy、HTTP/TUI/React 只读投影和 OpenAPI 隐私。全仓普通/race 测试、vet/staticcheck、模块一致性、`govulncheck`、strict TypeScript、16 项 Vitest、Vite production build、npm audit、凭据/运行产物扫描与确定性契约生成在提交前复核；真实 Provider、Shell、网络、Docker 和 Sandbox 执行仍关闭。

唯一推荐下一切片：继续 P7，在 schema v45 增加持久化操作者引导队列。Run 忙碌时不再只能拒绝追加要求，而是把有界输入按序落库，在安全 root turn 边界 exactly-once 投递，不打断活动模型/工具提交；复用 PendingInput、Coordinator inbox、lease、取消、来源和幂等语义。先提供只读队列投影，模型、HTTP、TypeScript 和 child 都不能伪造 operator authority。Specialist Skill 最小化作为其后的独立审计。
...
长对话恢复时依次阅读：`README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_STATUS.md`、本文件、`docs/TASK_BOOK.md`、`docs/http-api.md`、`docs/errors.md`、`docs/adr/0001-go-control-plane.md`、`docs/adr/0002-run-centric-runtime.md` 和 `docs/adr/0003-run-execution-modes.md`。
