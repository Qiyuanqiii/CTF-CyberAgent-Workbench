...
- CGO SQLite 驱动 `github.com/mattn/go-sqlite3`，当前 schema 版本为 v13。
...
- Tool Gateway 的 Artifact 捕获与真实 Docker 隔离；first-class ScriptProcess、逐次审批、Session Grant 和工具预算已完成。
...
1. 定义工具输出到 Artifact 的捕获阈值、SHA-256、MIME、截断关系和 Run 事件投影，避免大输出静默丢失。
2. 为 WorkItem/Note 增加低风险结构化工具提案，使模型修改经过 schema、scope、policy、预算、事件和显式控制边界。
3. 为 Session Grant 增加 TUI 审批提示和“批准一次/本会话”组合操作，同时保持 Go Store 为唯一授权源。
4. Docker/Local 真实命令执行继续关闭，直到 Sandbox 与 Artifact 边界同时通过审计。
