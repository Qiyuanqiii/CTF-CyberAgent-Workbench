...
P8 已开始：schema v35 已把完成的 Fan-out execution 投影为通用 `draft` Finding、不可变 `model_assertion` Evidence 和可重建的 Markdown/JSON Report；该投影不等于漏洞验证。
...
1. 为 schema v35 `draft` Finding 增加显式 operator validation transition，并接入不可变 Artifact-backed Evidence；在该信任边界完成前不输出 SARIF 高置信结论。
