...
1. schema v32 application 的独立审计已经通过；下一步设计显式 operator child schedule/continue 命令，只允许选择该 application 已 instructed 的 ready child，并复用 durable schedule、总预算、取消与恢复，模型仍不能直接启动。
