...
P6 当前已推进到 schema v66：v63 固定阻塞态 start-gate 审查，v64 只记录 `preview|docker|local` 非授权档位，v65 建立 16 项非授权生产证据 receipt，v66 再把 collector 调用放进 durable write-ahead attempt、摘要化 operation、过期 generation lease、当前代 reconciliation、类型化 failure 与原子 result 边界。SQL 阻止新的 v65 receipt 绕开 v66 result，历史 receipt 不伪造 attempt；恢复必须进入下一 generation，旧 worker 不能提交。当前 collector 与 quiescent checkpoint 都是零 daemon/零进程实现，checkpoint 不是生产资源核验，真实 Linux harness 与 start 生命周期继续关闭。
...
长对话恢复时依次阅读：`README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_STATUS.md`、本文件、`docs/TASK_BOOK.md`、`docs/http-api.md`、`docs/errors.md`，再按序阅读 `docs/adr/0001-*.md` 到 `docs/adr/0028-recoverable-docker-production-evidence-attempts.md`。
