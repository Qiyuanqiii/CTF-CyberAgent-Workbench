...
- 产品可用度：完整 Code + Cyber 产品约 61-65%；其中通用 Coding Agent 工作流约 56%，Cyber 自动化工作流约 20%。该指标衡量用户现在能够完成多少真实端到端任务。
- 上述数值是依据已测试任务切片给出的工程估算，不是性能基准，也不代表仍被安全关闭的功能已经可用。

V2 的 P0/P1 已完成，P2 已具备稳定的单 Agent 恢复、Provider streaming、进程内主动取消、schema v16 有界工具循环、schema v17 跨进程执行租约/心跳/fencing，以及 schema v18 独立 capability 的跨进程 root 活动模型取消。P3 主体已落地：WorkItem/schema v9、Note/schema v10、事务化关系与事件、完整 `todo`/`note` CLI、可见性、8192-token Context Builder，以及不含正文的持久化上下文来源审计。P4 已完成 schema v19-v38 的单 root Coordinator、最多两个核心 child 的审阅/申请/调度链，以及独立 1/2/4/6 只读 Fan-out。P5 已落地统一 Tool Gateway、持久审批、Session Grant、工具预算、ScriptProcess、来源绑定 Artifact、结构化记忆工具和可恢复 Provider 工具批次。P9 已新增 loopback-only `api.v1`、Go-owned OpenAPI、SSE/poll、Run-first TUI、React/Vite、Windows Wails 壳、schema-v72 受控 Run 创建、D1-S1/S2 Session 排队/取消，以及 schema-v73 幂等生命周期和最多八条冻结输入的 RunSupervisor 执行交接。真实 Local/Docker/Shell 进程、后台 wake/retry scheduler 和通用文件/审批/Skill Web mutation 继续关闭。
...
长对话恢复时依次阅读：`README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_STATUS.md`、本文件、`docs/TASK_BOOK.md`、`docs/http-api.md`、`docs/errors.md`，再按序阅读 `docs/adr/0001-*.md` 到 `docs/adr/0038-idempotent-run-control-and-bounded-handoff.md`。
