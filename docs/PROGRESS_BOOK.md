...
- V2 Run-centric Runtime：约 96%。
- 项目骨架和模块边界：约 99%。

V2 的 P0/P1 已完成，P2 已具备稳定的单 Agent 恢复、Provider streaming 和进程内主动取消链路。P3 主体已落地：WorkItem/schema v9、Note/schema v10、事务化关系与事件、完整 `todo`/`note` CLI、可见性、8192-token Context Builder，以及不含正文的持久化上下文来源审计。P5 已落地统一 Tool Gateway、schema v11 持久化幂等逐次审批，以及 schema v12 可撤销 Session Grant 与 Run 工具预算；真实命令执行继续关闭。
...
- CGO SQLite 驱动 `github.com/mattn/go-sqlite3`，当前 schema 版本为 v12。
...
- Tool Gateway 的 Artifact 捕获、first-class script-process proposal，以及真实 Docker 隔离；逐次审批、Session Grant 和工具预算已完成。
...
1. 把 `script_process.v1` 从 legacy Shell ToolRun 升级为独立 typed proposal，并让 Run 创建与提案提交具备幂等恢复语义。
2. 定义工具输出到 Artifact 的捕获阈值、哈希、MIME 和事件投影，避免截断内容静默丢失。
3. 为 WorkItem/Note 增加低风险结构化工具提案，使模型修改经过 schema、scope、policy、预算、事件和显式控制边界。
4. 为 Session Grant 增加 TUI 审批提示和“批准一次/本会话”组合操作，同时保持 Go Store 为唯一授权源。
5. Docker/Local 真实命令执行继续关闭，直到 Sandbox 与 Artifact 边界同时通过审计。
