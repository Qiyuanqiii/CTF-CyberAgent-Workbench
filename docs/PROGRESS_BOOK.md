...
P6 已完成三条纵向链路：schema v48 定义严格 `sandbox_manifest.v1` 并形成 metadata-only preparation；schema v49 复用通用审批账本，增加显式 request/review、完整 Manifest 重新提交、`os.Root` 工作区挂载解析和不可变 candidate；schema v50 再增加禁用态 `sandbox_execution.v1`、输入 Artifact 完整性快照、metadata-only 输出计划、独立 generation fencing、取消与清理恢复。原始命令、argv、路径、环境、密钥、目标、Manifest、Artifact 正文和 lease token 不落生命周期账本或事件。全部后端标志仍为 false，Local/Docker 和真实进程完全关闭。
...
- schema v48-v50 增加 `sandbox_manifest.v1`、严格 duplicate-aware JSON、共享审批、`sandbox_execution_candidate.v1` 与禁用态生命周期；CLI 可离线校验、准备、审批、重新提交、begin/cancel/cleanup。命令、argv、路径、环境值、密钥引用、目标、Manifest JSON、Artifact 正文和 lease token 不进入生命周期账本/事件；全部后端能力仍为 false。
...
本轮切片审计补强了 SQL 对 preparation 输入/输出计数和输出上限的直接绑定、清理重放的 lease 释放错误传播，以及取消/清理在提交结果不确定时的 operation-ledger 恢复。测试覆盖协议 fail-closed、完整生命周期与重放、终态 Run 清理、跨 Run Artifact 拒绝、租约活动互斥/续期/过期接管、旧 worker fencing、SQL 不可变、v49 原地升级、事件隐私和 CLI smoke；uncached 全仓普通/race、`go vet`、零告警 `staticcheck`、module verify/tidy diff、`govulncheck`、前端测试/typecheck/OpenAPI drift/生产构建、npm audit、凭证/运行产物/Runner 调用扫描、diff 检查和隔离真实二进制生命周期 smoke 全部通过，未发现未解决高/中风险问题。当前只待推送后 GitHub Actions 远端门禁确认。双指标更新为架构完成度约 98%，产品可用度仍约 45-50%，因为真实执行没有开放。
...
长对话恢复时依次阅读：`README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_STATUS.md`、本文件、`docs/TASK_BOOK.md`、`docs/http-api.md`、`docs/errors.md`，再按序阅读 `docs/adr/0001-*.md` 到 `docs/adr/0010-disabled-sandbox-lifecycle.md`。
