...
P6 当前已推进到 schema v67：v63 固定阻塞态 start-gate 审查，v64 只记录 `preview|docker|local` 非授权档位，v65 建立 16 项非授权生产证据 receipt，v66 把 collector 调用放进 durable write-ahead attempt 与 generation fencing，v67 再增加显式 opt-in 的 Linux 只读 daemon harness。该 harness 只有一次精确 label 容器清单 GET 与四次固定元数据 GET，不 pull、不写 daemon、不启动进程；16 项结果仍全部阻塞且生产验证数为零。
...
长对话恢复时依次阅读：`README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_STATUS.md`、本文件、`docs/TASK_BOOK.md`、`docs/http-api.md`、`docs/errors.md`，再按序阅读 `docs/adr/0001-*.md` 到 `docs/adr/0029-bounded-linux-read-only-docker-evidence-harness.md`。
