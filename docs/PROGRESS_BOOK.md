...
- schema v48-v52 增加 `sandbox_manifest.v1`、严格 duplicate-aware JSON、共享审批、`sandbox_execution_candidate.v1`、禁用态生命周期与后端/输出预检、仅模拟后端证据和原子内存输出事务；CLI 可离线校验、准备、审批、重新提交、begin/preflight/evidence/output-simulate/cancel/cleanup。命令、argv、路径、环境值、密钥引用、目标、Manifest JSON、Artifact 正文、输出 locator、夹具正文和 lease token 不进入公开账本/事件；全部生产后端能力仍为 false。
...
长对话恢复时依次阅读：`README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_STATUS.md`、本文件、`docs/TASK_BOOK.md`、`docs/http-api.md`、`docs/errors.md`，再按序阅读 `docs/adr/0001-*.md` 到 `docs/adr/0012-simulation-only-sandbox-evidence.md`。
