...
唯一推荐下一切片：继续 P7，以 schema v46 完成队列控制加固。增加 pending-only、摘要幂等的操作者取消事实，明确禁止编辑、重排或取消当前 prepared 消息；定义 idle/paused Run 的 wake/drain 语义，并允许普通 Session 调用方提供跨进程稳定重试标识。HTTP/React 继续只读，模型与 child 不获得队列写权限。其后再独立审计 Specialist Skill 最小化与 Code/Cyber Skill 分离。
...
长对话恢复时依次阅读：`README.md`、`docs/PROJECT_MEMORY.md`、`docs/PROJECT_STATUS.md`、本文件、`docs/TASK_BOOK.md`、`docs/http-api.md`、`docs/errors.md`、`docs/adr/0001-go-control-plane.md`、`docs/adr/0002-run-centric-runtime.md`、`docs/adr/0003-run-execution-modes.md`、`docs/adr/0004-plan-delivery-workflow.md` 和 `docs/adr/0005-operator-steering-queue.md`。
