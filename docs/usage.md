...
The non-schema Desktop pathless boundary reuses the same reader and parser behind a Go-native selector. After validation, Go forgets the path and body and exposes only a five-minute, one-time opaque handle. Desktop D0-A/D0-B connects that selector to a Wails native `.zip` dialog and a bounded React risk preview. The renderer still cannot provide a path or bytes, and no Desktop/HTTP installation control exists. ADR 0033 records the pathless boundary, ADR 0034 the visible shell, and ADR 0035 the lifecycle/event-resumption hardening.
