...
Provider failures are normalized as `retryable`, `rate_limited`, `invalid_response`, `cancelled`, or `permanent`. RunSupervisor retries only retryable transport/capacity outcomes, with three attempts by default, 100 ms exponential backoff, and a 2 second local wait ceiling. A server `Retry-After` longer than that ceiling is not shortened: the turn returns a rate-limit error and keeps its pending input for a later `run step`. Invalid lifecycle JSON, authentication/configuration failures, policy denial, and tool calls are not transport-retried.

Every call attempt emits `model.started` and then `model.completed` or `model.failed` in `run events`. Attempt numbers continue across process restart. Model terminal events and `execution_millis` commit together, and replaying the same terminal event cannot double-charge the budget. `run step` prints `model_attempts` and `model_outcome`. Exhausted transient retries return unavailable exit code 6, rate limits return resource-exhausted exit code 8, cancellation returns 7, and deadline expiration returns 9.
