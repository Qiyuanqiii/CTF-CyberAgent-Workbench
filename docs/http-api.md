...
- read token 可以读取该进程数据库暴露的全部只读资源；control token 只能调用十六条窄 mutation，不能读取资源。两者都应视为本地管理员凭据。
