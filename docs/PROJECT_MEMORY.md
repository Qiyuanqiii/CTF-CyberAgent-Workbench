...
- Database schema: v59.
- `README.md` carries the canonical bilingual schema timeline in strict `v1 -> v59` order. `internal/store/readme_history_test.go` binds its row count and ordering to `LatestSchemaVersion`, so a future migration cannot silently leave the public history missing or out of sequence.
...
Continue P6 with a separately gated schema-v60 runtime-input projection and never-started lifecycle design; do not reinterpret v59 handoff evidence as process-isolation or execution proof:

1. Decide how the verified `bundle.tar` is projected to exact Manifest input targets without an in-container bootstrap process, writable reviewed mount, or generic archive destination.
2. Persist a start intent only after input projection, resource/network controls, termination identity, and orphan ownership can be independently revalidated and recovered.
3. Design start/wait/TERM/KILL as a fixed endpoint state machine with generation fencing and cancellation fan-out. Keep output export and Artifact commit in a later atomic gate.
4. Run the v59 opt-in real-daemon handoff harness on Linux with an already-present exact image before allowing any later start gate to depend on it.
5. Broader network/secrets, HTTP/React mutation, Rust analyzers, and CTF solving remain deferred.

## Local Machine Note

The default `~/.cyberagent-workbench/cyberagent.db` currently carries a historical schema-v30 checksum that differs from this repository's immutable migration definition, so startup correctly fails closed with `migration 30 checksum or name mismatch`. The v59 slice did not modify migrations 1-58, and fresh/upgrade fixtures plus isolated `CYBERAGENT_HOME` runs pass. Preserve that local database for backup/diagnosis; do not delete it or rewrite `schema_migrations` automatically.
