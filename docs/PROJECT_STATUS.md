...
- Add recoverable application for schema v31 approved proposals. Application must revalidate Policy, parent Skills, child capacity, and current Run budgets, idempotently enter the existing admission path, and deliver one strict parent instruction per admitted child; expiry, restart, and partial failure must not leave ghost children. The model must still be unable to approve, apply, admit, spawn, or schedule.
