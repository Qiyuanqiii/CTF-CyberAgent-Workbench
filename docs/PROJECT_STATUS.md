...
- Define a strict `specialist_delegation.v1` proposal through which root may suggest at most two bounded goals, Skill subsets, and budget reservations. Persist and validate the proposal in Go, but keep actual admission/spawn behind an internal or operator policy in the first slice.
