...
Latest implemented batch: D1-G11/V10/R8 on schema v84. It adds bounded previous/next navigation inside the existing paired exact-redacted base/head preview, one append-only digest- and sequence-bound operator review for each verification snapshot receipt with every acceptance/authority semantic false, and canonical golden compatibility vectors for normal-empty and forced-timeout runner evidence sets. The three-slice functional gate and the full six-slice robustness gate are green after five low-risk correctness and documentation fixes, with no known unresolved high/medium issue on an enabled path. ADR 0059 is authoritative.
...
Candidate slices are D1-G12 keyboard-accessible paired-preview close/navigation,
D1-V11 metadata-only receipt-review projection in Code Handoff/export, and R9 strict
compatibility rejection vectors for malformed or future evidence-set receipts. Keep the Local profile disabled until a real
...
requires a later independent release gate; R2-R8 test-binary conformance and post-reap
