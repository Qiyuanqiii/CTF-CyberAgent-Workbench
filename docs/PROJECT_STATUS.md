...
- The independent schema v32 audit is complete. Next, add an explicit operator child schedule/continue command that can select only instructed ready children from that application and must reuse the existing durable scheduler, aggregate budgets, cancellation, and recovery. Models must still be unable to start or select child execution.
