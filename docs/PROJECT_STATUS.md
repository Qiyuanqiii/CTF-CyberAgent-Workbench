...
- Overall product vision: about 93%.
...
- Schema v19-v25 Agent Coordinator with stable root identity, idempotent inbox operations, strict wake/dependency semantics, explicit internal-only Specialist admission, validated same-Run Agent ownership for WorkItems/Notes, exact-attempt CompletionReports, a default-disabled Specialist Attempt Runtime, and two-phase exactly-once root inbox context. A policy permits at most two depth-one children with parent-Skill subsets, dedicated Sessions, reserved budgets, lease-fenced turn scheduling, exactly-once usage accounting, redacted crash notifications, takeover recovery, lifecycle interruption, SHA-256-backed recovery snapshots, and atomic Supervisor/Run integration. Root context admits at most four protocol-backed direct-child messages per turn without model-controlled cursors or sender selection. Default Coordinators cannot admit or execute children, and no child model loop exists.
...
- Versioned SQLite migrations through schema v25: legacy baseline, run-centric foundation, Run/Session projection constraints, legacy Task mapping, Supervisor checkpoints, cumulative model budgets, durable pending input, restart-safe protocol repair, Work Board, Notes, durable per-call approvals, revocable Session grants, atomic tool budgets, typed script processes, Run tool-output Artifacts, idempotent structured-memory operations, durable Supervisor tool rounds/calls, Run execution leases with checkpoint fencing, cross-process model cancellation, the bounded Coordinator, hashed inbox idempotency, hashed Specialist admission, same-Run Agent-owned memory, attempt-bound Specialist CompletionReports, lease-fenced Specialist Attempt history, and root inbox context delivery; each version is checksummed and transactional.
...
- Root and Specialist Note visibility is AgentNode-backed, but child-owned memory is not yet injected into a child model because the scheduler remains disabled.
...
Continue P4 from the bounded admission, Agent-owned memory, CompletionReport, Specialist Attempt, and root inbox context foundation:

- Route one internal no-tool Specialist model turn through the existing Provider/cancellation path and schema v24 usage accounting, then return its durable result through schema v25 while keeping public/model spawn unavailable.
- Define a strict child-turn application protocol whose usage, continue, finish, and crash decisions remain Go-owned, and test cancellation, restart, lease takeover, and budget exhaustion before enabling any concurrency.
