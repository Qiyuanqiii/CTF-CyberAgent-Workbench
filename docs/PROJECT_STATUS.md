...
- Schema v19-v27 Agent Coordinator with stable root identity, idempotent inbox operations, strict wake/dependency semantics, explicit internal-only Specialist admission, validated same-Run Agent ownership for WorkItems/Notes, exact-attempt CompletionReports, a default-disabled Specialist Attempt Runtime, two-phase exactly-once root and Specialist instruction context, and one explicitly invoked internal no-tool Specialist model turn. A policy permits at most two depth-one children with parent-Skill subsets, dedicated Sessions, reserved budgets, lease-fenced turn scheduling, exactly-once usage accounting, redacted crash notifications, takeover recovery, lifecycle interruption, SHA-256-backed recovery snapshots, and atomic Supervisor/Run integration. Schema v26 atomically records child model terminal state, usage, Policy, and allowed redacted Session messages under the same lease fence. Schema v27 selects strict direct-parent instructions plus active child-owned WorkItems/Notes under independent token/byte bounds, commits consumption only with child lifecycle success, and preserves pending instructions across crash, interruption, and takeover. Public/model spawn and autonomous or concurrent child loops remain unavailable.
...
Continue P4 from the bounded admission, Agent-owned memory, CompletionReport, Specialist Attempt, root/child context ledgers, and internal no-tool child-turn foundation:
