...
- Schema v19-v26 Agent Coordinator with stable root identity, idempotent inbox operations, strict wake/dependency semantics, explicit internal-only Specialist admission, validated same-Run Agent ownership for WorkItems/Notes, exact-attempt CompletionReports, a default-disabled Specialist Attempt Runtime, two-phase exactly-once root inbox context, and one explicitly invoked internal no-tool Specialist model turn. A policy permits at most two depth-one children with parent-Skill subsets, dedicated Sessions, reserved budgets, lease-fenced turn scheduling, exactly-once usage accounting, redacted crash notifications, takeover recovery, lifecycle interruption, SHA-256-backed recovery snapshots, and atomic Supervisor/Run integration. Schema v26 atomically records child model terminal state, usage, Policy, and allowed redacted Session messages under the same lease fence. Public/model spawn and autonomous or concurrent child loops remain unavailable.
...
- Root and Specialist Note visibility is AgentNode-backed. The schema v26 child turn currently receives its Mission, assigned Skills, budget counters, and bounded child Session history; child-owned WorkItems/Notes and parent instruction inbox are not yet injected.
...
Continue P4 from the bounded admission, Agent-owned memory, CompletionReport, Specialist Attempt, root inbox context, and internal no-tool child-turn foundation:

- Add a recoverable child-context ledger for parent instructions, child-owned WorkItems/Notes, and bounded inbox content; sender, owner, and consumption remain Go-owned.
- Add a Go-internal scheduler for at most two children with bounded concurrency, cancellation fan-out, stop conditions, and aggregate token/time checks before exposing any spawn entry.
- Add one bounded child lifecycle repair with counters independent from transport retries; raw invalid output must not enter Session history, events, or the repair prompt.
