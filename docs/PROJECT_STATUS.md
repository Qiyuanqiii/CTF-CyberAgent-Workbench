...
- Add an independent operator/internal review fact and recoverable application for schema v30 proposals. Approval must revalidate Policy, parent Skills, child capacity, and current Run budgets, idempotently enter the existing admission path, and deliver one strict parent instruction per admitted child; denial, expiry, restart, and partial failure must not leave ghost children. The model must still be unable to approve, admit, spawn, or schedule.
