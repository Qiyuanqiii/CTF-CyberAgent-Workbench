...
P7 now includes the Go-owned `skill.v1` Registry, immutable Run-scoped version selection, durable metadata-only provenance, bounded root-context delivery, schema v41 execution modes, and schema v42's three-direction Plan/Delivery proposal and operator-selection projection into existing WorkItems/Notes. The next slice adds Delivery-phase per-slice audit and handoff gates. Declared Skill dependencies, execution modes, and Plan selection do not alter Tool Gateway or Policy authority.
