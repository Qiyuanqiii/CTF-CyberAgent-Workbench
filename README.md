...
P7 now includes the Go-owned `skill.v1` Registry, immutable Run-scoped version selection, durable metadata-only Skill provenance, bounded root-context delivery, schema v41 execution modes, schema v42's three-direction Plan/Delivery proposal and operator-selection projection, schema v43 provenance separation, and schema v44 immutable per-slice Delivery audit/handoff gates. The next schema v45 slice adds a durable operator steering queue delivered at safe turn boundaries without interrupting active tool calls. Declared Skill dependencies, execution modes, Plan selection, document content, and audit assertions do not alter Tool Gateway or Policy authority.
