...
The first React/Vite read console now lives under `web/`. It generates TypeScript DTOs from `docs/openapi.json`, reads Runs, Sessions, WorkItems, Notes, Artifact descriptors, ToolRounds, budgets, and leases, and consumes resumable SSE with authenticated `fetch`. A production build can be hosted on the same loopback origin by `cyberagent api serve --ui-dir web/dist`; Go loads the validated bundle into an immutable startup snapshot and enforces bounded HTML fallback, immutable hashed-asset caching, and a strict CSP. The read token stays in page memory and never enters a URL, localStorage, or sessionStorage. The browser has no control token and does not duplicate Go policy. The loopback Vite proxy remains available for frontend development.
