...
- **发现项与报告 / Findings and reports:** immutable model-assertion provenance, deterministic exact-fact deduplication, Artifact-backed operator validation/rejection, stable source projection digests, and Go-rendered Markdown/JSON.
...
P8 now includes schema v35's deterministic Finding/Evidence/Report projection and schema v36's immutable, same-Run Artifact Evidence plus one-time operator `validated/rejected` decisions. Source projection digests remain stable across validation. Acceptance/fix transitions, SARIF, and CI annotations remain future slices.
