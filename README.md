...
- **Agent Coordinator:** stable root identity, atomic Run/Supervisor status projection, bounded durable inboxes, hashed exactly-once send/finish intent, explicit wake/dependency semantics, internal-only two-child admission, lease-fenced child Attempt scheduling and usage accounting, crash/takeover recovery, attempt-bound CompletionReports, two-phase exactly-once root and Specialist instruction context, child-owned memory selection, and one opt-in no-tool Specialist turn with a durable model ledger; public spawn and autonomous/concurrent child loops remain disabled.
