...
- **发现项与报告 / Findings and reports:** immutable model-assertion provenance, deterministic exact-fact deduplication, draft validation state, stable projection digests, and Go-rendered Markdown/JSON.
...
P8 now includes schema v35's first generic Finding/Evidence/Report slice with immutable draft findings, source-bound model-assertion evidence, deterministic projection digests, and Markdown/JSON rendering. Validation transitions, SARIF, CI annotations, and Artifact-backed non-model evidence remain future slices.
