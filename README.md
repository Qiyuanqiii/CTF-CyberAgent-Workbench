...
- **Agent Coordinator:** stable root identity, atomic Run/Supervisor status projection, bounded durable inboxes, hashed exactly-once send/finish intent, explicit wake/dependency semantics, internal-only two-child admission and concurrent scheduling, shared-lease cancellation fan-out, aggregate Run budget reconciliation, lease-fenced child Attempts, crash/takeover recovery, attempt-bound CompletionReports, two-phase exactly-once root and Specialist instruction context, child-owned memory selection, and a durable no-tool Specialist model ledger; public/model spawn and autonomous scheduling remain disabled.
