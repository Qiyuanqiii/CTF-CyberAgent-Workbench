...
- **Agent Coordinator:** stable root identity, atomic Run/Supervisor status projection, bounded durable inboxes, hashed exactly-once send/finish intent, explicit wake/dependency semantics, internal-only two-child admission with Skill/budget constraints, attempt-bound CompletionReports, lifecycle cascade, and restart-validated graph snapshots; child execution remains disabled.
