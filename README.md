...
只读 Run Event Stream 也已接入同一控制面。`/api/v1/runs/{run_id}/events/stream` 使用 SSE 推送 SQLite 中已脱敏的持久化事件；每帧 `id` 是与 Run 绑定的不透明 cursor，客户端可通过 `Last-Event-ID` 精确续传。连接具有批量、帧大小、总事件数、寿命、写超时和进程并发上限，心跳不伪造 Run event；服务器关闭会主动取消长连接。它不推送模型正文、不接受 token query、不提供取消或其他写操作。
...
- **可恢复运行 / Resumable runs:** durable checkpoints, cross-process execution leases with heartbeat/fencing, bounded execution, restart recovery, graceful terminal cancellation, and explicit lifecycle actions.
...
- **可扩展架构 / Extensible architecture:** Go control plane with a generated OpenAPI 3.1 contract, loopback-only read API/SSE stream, and planned TypeScript UI, Docker sandbox, and Rust analyzer boundaries.
...
当前仓库尚未选择开源许可证，也没有 `LICENSE` 文件。公开可见不等于授权使用；在许可证加入前，默认版权规则生效，项目所有者保留全部权利。GitHub 用户仍可依据平台条款查看和 fork，但没有从本仓库获得复制、修改或再发布代码的一般许可。许可证必须由项目所有者明确选择，当前实现不会擅自添加。

No open-source license has been selected and the repository has no `LICENSE` file. Public visibility is not a general grant to use the code: default copyright applies and the project owner retains all rights until a license is explicitly added. GitHub users may still view and fork under the platform terms, but this repository currently grants no general permission to copy, modify, or redistribute the code. License selection remains an explicit owner decision.

## Development Priority

The current priority is the V2 run-centric runtime. P0 and P1 are complete. P2 supports resumable root Agent turns, cumulative token/model-time accounting, bounded execution and Provider retry loops, strict Supervisor-owned `continue`, `finish`, and `wait` actions, one Run execution path for ordinary CLI/TUI Session chat, real Provider streaming with bounded `model.delta` progress, application-owned active-call query/cancellation, Bubble Tea live metadata and `Ctrl+X` cancellation, durable model events, exactly one restart-safe lifecycle-protocol repair, the schema v16 bounded structured-memory tool loop, and schema v17 cross-process execution leases with heartbeat and fencing. P3 includes migration v9 WorkItems, migration v10 Notes, transactional relationships/events, `todo` and `note` CLI lifecycles, root/owner visibility, token-budgeted memory selection, and durable context provenance. P5 includes the unified Tool Gateway, trusted workspace scope binding, schema v11 durable per-call approvals, schema v12 revocable Session grants and atomic Run tool-call accounting, schema v13 first-class typed script processes, schema v14 source-bound hashed output Artifacts, schema v15 idempotent structured-memory mutations, and v16 durable Provider tool rounds. P9 now includes the authenticated loopback-only `api.v1` read surface, token-free execution-lease status, a deterministic Go-generated OpenAPI 3.1 contract, and a bounded resumable SSE Run-event projection; cross-process cancellation and TypeScript remain pending. Real Local/Docker command execution and all non-memory model tools remain disabled. CTF-specific solving logic stays deferred until the generic runtime is stable.
