...
P7 Skills 的第一条纵向链路已经落地。Go 内置并严格校验 `code`、`review`、`learn`、`script` 四份 `skill.v1` 元数据，包括固定版本、兼容 Profile、工具依赖、相对内容路径、UTF-8、字节数、保守 token 上界和 SHA-256。只读 Registry 与 `skill list/show/validate` 不创建数据库，也不读取任意外部路径；Skill 内容当前不会进入模型上下文，工具依赖只是前置声明，绝不授予执行权限。

P7 的第二条纵向链路新增 schema v39 与不可变 `skill_selection.v1`。操作者可在 Run 启动前，用 `skill select` 为其 Profile 固定最多 8 个 Skill 的名称、版本、内容哈希和总保守 token 预算；同一 Run 只能有一份选择，原始 operation key 只以域分隔 SHA-256 摘要落库，并发请求和跨重启重放会收敛到同一结果。事件只记录数量与预算，不记录 Skill 正文、路径、工具依赖或原始 key。该选择当前仍不注入 Provider prompt，也不授予任何工具、Shell、网络或子 Agent 权限。
...
The first P7 Skills vertical slice is now in place. Go embeds and strictly validates four `skill.v1` metadata records for `code`, `review`, `learn`, and `script`, including pinned versions, compatible Profiles, tool dependencies, relative content paths, UTF-8, byte counts, a conservative token upper bound, and SHA-256. The read-only Registry and `skill list/show/validate` create no database and accept no arbitrary external path. Skill content is not injected into model context, and a tool dependency is only a prerequisite declaration, never a capability grant.

The second P7 vertical slice adds schema v39 and immutable `skill_selection.v1`. Before a Run starts, an operator can use `skill select` to pin up to eight Profile-compatible Skills by name, version, content hash, and aggregate conservative token budget. A Run can have only one selection; raw operation keys are stored only as domain-separated SHA-256 digests, and concurrent or post-restart replays converge on the same result. Events contain counts and budgets only, never Skill content, paths, tool dependencies, or raw keys. A selection still injects nothing into Provider prompts and grants no tool, Shell, network, or child-Agent capability.
...
- **Skill Registry 与 Run 选择 / Skill Registry and Run selection:** Go-owned strict `skill.v1` manifests plus one immutable, version/hash-pinned, token-budgeted `skill_selection.v1` per Run; no prompt injection or capability grant yet.
...
P7 now includes the first Go-owned `skill.v1` boundary: strict bounded manifests, embedded content integrity, Profile/tool-dependency compatibility, a mutation-free Registry, and read-only CLI inspection. Run-scoped version selection, durable provenance, hot loading, and model-context injection remain pending; declared dependencies do not alter Tool Gateway or Policy authority.
