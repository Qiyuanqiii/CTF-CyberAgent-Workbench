...
当前 Go 内部已增加 `SpecialistScheduler`：一次调度持有一份 Run execution lease，每轮最多并发两个显式指定的 ready child，并以 32 轮硬上限、全完成/无 ready/首错/父取消/token/执行时间等条件停止。父 context、lease 心跳故障或任一 child 不可继续错误会取消同轮其他 child；调度器等待全部 Attempt 先写入终态，再释放 lease。Run 总预算每轮前后都从 SQLite 复核：root token/执行时间来自 Supervisor checkpoint，child token 同时核对 Agent 投影与 Attempt 账本，全部 child 模型耗时从持久化 model-call ledger 求和。该调度器仍只有 Go 内部构造路径，不新增 CLI、HTTP、模型 spawn、工具、Shell 或网络权限。
...
当前 v36 runtime 还提供不写库、不调用 Provider 的 SARIF 2.1.0 与 CI 门禁投影。`report show --format sarif` 默认只把人工验证过的 Finding 放入 SARIF `results`；草稿和已拒绝项仅保留为汇总计数，避免 GitHub Code Scanning 忽略 `result.kind` 后误报。稳定 rule、工作区相对 URI 和 Finding fingerprint 用于跨运行识别。`report check` 默认只在 `validated` 且至少为 `high` 时返回退出码 4；只有显式指定 `--fail-status active` 才会把草稿纳入门禁。`accepted/fixed` 与平台专用 CI annotations 仍待后续实现。
...
The current v36 runtime also provides read-only SARIF 2.1.0 and CI-gate projections without a database write or Provider call. `report show --format sarif` includes only operator-validated Findings in SARIF `results`; drafts and rejected claims remain summary counts so GitHub Code Scanning cannot promote them when it ignores unsupported `result.kind` values. Stable rules, workspace-relative URIs, and Finding fingerprints support cross-run identity. `report check` defaults to exit code 4 only for validated Findings at or above `high`; drafts enter the gate only through explicit `--fail-status active`. `accepted/fixed` and platform-specific CI annotations remain future work.
...
- **发现项与报告 / Findings and reports:** immutable model-assertion provenance, deterministic exact-fact deduplication, Artifact-backed operator validation/rejection, stable source projection digests, Go-rendered Markdown/JSON, validated-only SARIF 2.1.0, and an explicit CI gate.
...
> [!NOTE]
> 当前版本仍在积极开发中。Provider 只能自动创建 WorkItem/Note；双 child 并发仅存在于未公开的 Go 内部调度器。真实 Shell/Docker 执行、更广工具面、公开或模型自主子 Agent 调度、Web UI 和 CTF 自动求解尚未开放。<br>
...
P8 now includes schema v35's deterministic Finding/Evidence/Report projection, schema v36's immutable same-Run Artifact Evidence plus one-time operator `validated/rejected` decisions, validated-only SARIF 2.1.0 export, and a typed read-only CI gate. Source projection digests remain stable across validation. Acceptance/fix transitions and platform-specific CI annotations remain future slices.
