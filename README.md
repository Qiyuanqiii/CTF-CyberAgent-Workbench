...
Prayu 已完成内置浏览器的第一批 Go 主控安全边界，但**当前还没有启动 Chromium、访问网络或向用户提供可操作的浏览器窗口**。现有 Wails WebView2 仍只负责渲染桌面界面，不会被复用为 CTF 浏览器，也不会加载靶场页面。

The first Go-owned safety boundary for an Agent browser is complete, but **Prayu does not yet launch Chromium, access a target, or expose an interactive browser window**. The existing Wails WebView2 remains a Desktop renderer and is not reused for adversarial CTF pages.
...
`browser_session_plan.v1` 当前仍固定 `start_blocked=true`，且 process/network/profile-write/request-mutation/replay/Artifact authority 全为 `false`。下一批将实现浏览器可执行文件发现与摘要、受控一次性 Profile 生命周期以及 Disabled/Fake CDP transport；真实导航只会在后续独立启动审查通过后开放。边界见 [ADR 0069](docs/adr/0069-go-owned-browser-profiles-target-scope-and-session-plan.md)。

`browser_session_plan.v1` currently fixes `start_blocked=true`, with process, network, profile-write, request-mutation, replay, and Artifact authority all `false`. The next batch covers executable discovery and identity, disposable-profile lifecycle, and a Disabled/Fake CDP transport. Real navigation remains behind a later independent launch review. See [ADR 0069](docs/adr/0069-go-owned-browser-profiles-target-scope-and-session-plan.md).
