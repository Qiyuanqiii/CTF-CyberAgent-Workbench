...
P7 now includes the Go-owned `skill.v1` Registry, immutable Run-scoped version selection, durable metadata-only Skill provenance, bounded root-context delivery, schema v41 execution modes, schema v42's three-direction Plan/Delivery proposal and operator-selection projection, schema v43 provenance separation, schema v44 immutable per-slice Delivery audit/handoff gates, schema v45-v46 durable operator steering and controls, and schema v47 minimal parent-selected Specialist Skill delivery with distinct Code/Cyber rules. The next slice establishes the Go-owned Sandbox Manifest and immutable execution-intent boundary while keeping real Local/Docker execution disabled. Declared Skill dependencies, execution modes, Plan selection, document content, audit assertions, queued input, and Sandbox descriptions do not alter Tool Gateway or Policy authority.
