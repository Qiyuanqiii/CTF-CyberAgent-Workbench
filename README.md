...
P7 now includes the Go-owned `skill.v1` Registry, immutable Run-scoped version selection, durable metadata-only Skill provenance, bounded root-context delivery, schema v41 execution modes, schema v42's three-direction Plan/Delivery proposal and operator-selection projection, schema v43 provenance separation, schema v44 immutable per-slice Delivery audit/handoff gates, and schema v45 durable operator steering delivered exactly once at safe turn boundaries. The next schema v46 slice adds pending-only cancellation and explicit queue-drain controls without widening model, HTTP, or child authority. Declared Skill dependencies, execution modes, Plan selection, document content, audit assertions, and queued input do not alter Tool Gateway or Policy authority.
