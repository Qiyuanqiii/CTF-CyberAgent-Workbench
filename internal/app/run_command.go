...
func (a *App) runSupervisorStep(ctx context.Context, args []string) error {
	...
	fmt.Fprintf(a.out, "run %s turn %d completed\nattempt: %s\nrecovered: %t\nmodel_attempts: %d\nprotocol_repairs: %d\nmodel_outcome: %s\naction: %s\nrun_status: %s\nprovider: %s\nmodel: %s\nusage: input=%d output=%d total=%d\ncumulative_tokens: %d\nexecution_millis: %d\nnext_turn: %d\nresponse: %s\n",
		result.Handle.RunID, result.Turn, result.AttemptID, result.Recovered, result.ModelAttempts, result.ProtocolRepairs, result.ModelOutcome, result.Action.Kind, result.RunStatus,
...
func (a *App) runSupervisorExecute(ctx context.Context, args []string) error {
	...
	for _, step := range result.Steps {
		fmt.Fprintf(a.out, "turn %d\t%s\t%s/%s\tattempts=%d\trepairs=%d\ttokens=%d\tnext=%d\n",
			step.Turn, step.Action.Kind, step.Provider, step.Model, step.ModelAttempts, step.ProtocolRepairs, step.Usage.TotalTokens, step.Checkpoint.NextTurn)
	}
	if err != nil {
